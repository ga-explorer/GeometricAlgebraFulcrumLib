...
{
    public sealed class GrParametricCurveTreeBranch3D :
        GrParametricCurveTreeNode3D
    {
        public override int Count => 2;

        public GrParametricCurveTreeNode3D Child0 { get; private set; }
        
        public GrParametricCurveTreeNode3D Child1 { get; private set; }
        ...
        internal GrParametricCurveTreeBranch3D(GrParametricCurveTree3D parentTree) 
            : base(parentTree)
        {
        }

        internal GrParametricCurveTreeBranch3D(GrParametricCurveTreeBranch3D parentBranch, bool isRightChild)
            : base(parentBranch, isRightChild)
        {
        }


        private GrParametricCurveTreeBranch3D CreateBranchChildren(GrParametricCurveTreeOptions3D options)
        {
            Child0 = new GrParametricCurveTreeBranch3D(this, false).GenerateTree(options);
            Child1 = new GrParametricCurveTreeBranch3D(this, true).GenerateTree(options);
        ...
        internal GrParametricCurveTreeNode3D GenerateTree(GrParametricCurveTreeOptions3D options)
        {
            // Update normals of second frame based on first frame
            if (ParentTree.FrameSamplingMethod == GrCurveFrameSamplingMethod.MinimizedRotation)
            ...
            return ParentTree.AddLeafNode(
                new GrParametricCurveTreeLeaf3D(ParentBranch, IsRightChild)
            );
        }

        internal GrParametricCurveTreeNode3D GetChildContaining(double parameterValue)
        {
        ...
        internal GrParametricCurveTreeNode3D GetChildContainingLength(double length)
        {
