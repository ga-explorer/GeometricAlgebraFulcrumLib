...
using GraphicsComposerLib.GraphViz.Dot.Image;
...
using GraphicsComposerLib.GraphViz.LaTeX;
    ...
    public class DotGraph : IDotGraph
    {
        ...
        public DotGraphLaTeXImage SetLaTeXImage(string imageId, string latexMath)
        {
            ...
            if (ImagesCache.TryGetValue(imageId, out var image))
            {
                ...
                {
                    latexImage.LaTeXMath = latexMath;
        ...
        public void SaveDotCode(string filePath)
        {
            ...
            var inputsArray = 
                imagesArray.Select(
                    i => Tuple.Create(i.ImageId, i.LaTeXMath)
                ).ToArray();

            var latexRenderer = new LaTeXRenderer()
            {
                WorkingPath = folderPath
            };

            latexRenderer.RenderMathToPngFiles(inputsArray, 150);
