...
namespace GeometricAlgebraFulcrumLib.MathBase.SignalAlgebra
{
    public class VectorFourierInterpolator
    {
        ...
        private static VectorFourierInterpolator Create(List<Complex[]> complexSamples, int vSpaceDimensions, double samplingRate, IEnumerable<int> frequencyIndexList)
        {
...

            {
                ...
                interpolator.SetTerm(
                    freqRad,
                    metric.CreateVector(cosVectorScalars),
                    metric.CreateVector(sinVectorScalars)
                );
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static VectorFourierInterpolator Create(XGaVector<ScalarSignalFloat64> signalSamples, IEnumerable<int> frequencyIndexList)
        {
            var vSpaceDimensions = 
        ...
        internal static VectorFourierInterpolator Create(XGaVector<ScalarSignalFloat64> signalSamples, double energyThreshold = 0.998d)
        {
            var frequencyIndexSet = GetDominantFrequencyIndexSet(
        ...
        public XGaFloat64Vector GetVector(double parameterValue)
        {
            return _termsDictionary.Values.Aggregate(
                SampleProcessor.CreateZeroVector(), 
                (current, term) => 
