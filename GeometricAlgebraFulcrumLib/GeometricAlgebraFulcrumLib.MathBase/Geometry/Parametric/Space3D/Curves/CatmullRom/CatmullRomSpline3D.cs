...
using GeometricAlgebraFulcrumLib.MathBase.Geometry.Parametric.Space1D.Curves.CatmullRom;
using GeometricAlgebraFulcrumLib.MathBase.Geometry.Parametric.Space3D.Frames;
using GeometricAlgebraFulcrumLib.MathBase.LinearAlgebra.Float64.Vectors.Space3D;
using GeometricAlgebraFulcrumLib.MathBase.ScalarAlgebra;
using MathNet.Numerics;

namespace GeometricAlgebraFulcrumLib.MathBase.Geometry.Parametric.Space3D.Curves.CatmullRom
{
    /// <summary>
        ...
        IParametricC2Curve3D
    {
        ...
        internal CatmullRomSpline3D(IEnumerable<IFloat64Vector3D> inputPointList, CatmullRomSplineType curveType, bool isClosed)
        {
            ...
            else
            {
                ...
                endPoint1 = 2 * _pointList[0].ToVector3D() - _pointList[1];
                endPoint2 = 2 * _pointList[^1].ToVector3D() - _pointList[^2];
            }
            ...
            {
                var vector = _pointList[i].ToVector3D() - _pointList[i - 1];
        ...
        public Float64Vector3D GetPoint(double parameterValue)
        {
            // Handle edge cases
            if (parameterValue <= _knotList[0])
                return _pointList[0].ToVector3D();

            if (parameterValue >= _knotList[^1])
                return _pointList[^1].ToVector3D();

            var (index1, index2) =
                GetKnotIndexContaining(parameterValue, 0, _knotList.Length - 1);

            if (index1 == index2)
                return _pointList[index1].ToVector3D();
        ...
        public Float64Vector3D GetDerivative1Point(double parameterValue)
        {
            if (parameterValue is <= 0d or >= 1d)
                return Float64Vector3D.Create(Differentiate.FirstDerivative(GetPointX, parameterValue),
        ...
        {
            if (parameterValue is <= 0d or >= 1d)
                return Float64Vector3D.Create(Differentiate.SecondDerivative(GetPointX, parameterValue),
