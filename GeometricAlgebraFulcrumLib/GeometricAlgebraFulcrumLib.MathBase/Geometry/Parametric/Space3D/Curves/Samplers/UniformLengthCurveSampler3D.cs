    ...
    IParametricCurveSampler3D
{
    ...
    public UniformLengthCurveSampler3D(IArcLengthCurve3D curve, Float64Range1D parameterRange, int count, bool isPeriodic = false)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public UniformLengthCurveSampler3D SetCurve(IArcLengthCurve3D curve, Float64Range1D parameterRange, int count, bool isPeriodic)
    {
        if ((isPeriodic && count < 1) || (!isPeriodic && count < 2))
            throw new ArgumentOutOfRangeException(nameof(count));
