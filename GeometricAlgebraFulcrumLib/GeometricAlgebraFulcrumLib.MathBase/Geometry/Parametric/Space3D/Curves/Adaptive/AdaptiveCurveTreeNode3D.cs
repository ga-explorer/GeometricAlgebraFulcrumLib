...
namespace GeometricAlgebraFulcrumLib.MathBase.Geometry.Parametric.Space3D.Curves.Adaptive
{
        ...
        IReadOnlyCollection<AdaptiveCurveTreeNode3D>
    {
        ...
        public bool HasNearEdgeFrames(AdaptiveCurveSamplingOptions3D options)
        {
            ...
            var angle = Frame0.Normal1.GetAngle(Frame1.Normal1).Radians;
            if (angle > angleMax) return false;

            angle = Frame0.Normal2.GetAngle(Frame1.Normal2).Radians;
            if (angle > angleMax) return false;

            angle = Frame0.Tangent.GetAngle(Frame1.Tangent).Radians;
            if (angle > angleMax) return false;
