        ...
        IParametricC2Curve3D
    {
        ...
        internal CatmullRomSpline3D(IEnumerable<IFloat64Tuple3D> inputPointList, CatmullRomSplineType curveType, bool isClosed)
        {
            ...
            else
            {
                ...
                endPoint1 = 2 * _pointList[0].ToTuple3D() - _pointList[1];
                endPoint2 = 2 * _pointList[^1].ToTuple3D() - _pointList[^2];
            }
        ...
        public Float64Tuple3D GetPoint(double parameterValue)
        {
            // Handle edge cases
            if (parameterValue <= _knotList[0])
                return _pointList[0].ToTuple3D();

            if (parameterValue >= _knotList[^1])
                return _pointList[^1].ToTuple3D();

            var (index1, index2) =
                GetKnotIndexContaining(parameterValue, 0, _knotList.Length - 1);

            if (index1 == index2)
                return _pointList[index1].ToTuple3D();
