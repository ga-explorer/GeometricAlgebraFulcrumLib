...
namespace GeometricAlgebraFulcrumLib.MathBase.BasicMath.Matrices
{
    public static class MatrixUtils
    {
            ...
            for (var index = 0UL; index < kVectorDimension; index++)
            {
                var vectorList = 
                    BasisBladeUtils
                        ...
                        .PatternToPositions()
                        .Select(i => metric.CreateVector(columnList[i].ScalarArray))
                        .ToArray();
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static LinFloat64VectorToVectorRotationSequence GetVectorToVectorRotationSequence(this Matrix<double> matrix)
        {
            Debug.Assert(
                matrix.Determinant().IsNearOne()
            );

            var rotation = LinFloat64VectorToVectorRotationSequence.Create(matrix.RowCount);
