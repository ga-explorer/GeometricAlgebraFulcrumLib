using GeometricAlgebraFulcrumLib.MathBase.Graphics.SdfShapes.Operations;
using GeometricAlgebraFulcrumLib.MathBase.Graphics.SdfShapes.Primitives;
using GeometricAlgebraFulcrumLib.MathBase.Graphics.SdfShapes.Transforms;
...
namespace GeometricAlgebraFulcrumLib.MathBase.Graphics.SdfShapes
{
    ...
    {
        public static Float64Vector3D ToTuple3D(this Color color)
        {
            var c = color.ToPixel<Rgb24>();

            return Float64Vector3D.Create(c.R / 255.0d,
                c.G / 255.0d,
                c.B / 255.0d);
        }
        ...
        public static Float64Vector3D ClampTo(this IFloat64Tuple3D tuple, IFloat64Tuple3D maxTuple)
        {
            return Float64Vector3D.Create(tuple.X.ClampTo(maxTuple.X),
                tuple.Y.ClampTo(maxTuple.Y),
                tuple.Z.ClampTo(maxTuple.Z));
        }

        public static Float64Vector3D ClampTo(this IFloat64Tuple3D tuple, IFloat64Tuple3D minTuple, IFloat64Tuple3D maxTuple)
        {
            return Float64Vector3D.Create(tuple.X.ClampTo(minTuple.X, maxTuple.X),
                tuple.Y.ClampTo(minTuple.Y, maxTuple.Y),
                tuple.Z.ClampTo(minTuple.Z, maxTuple.Z));
        }

        public static Float64Vector3D ClampToSymmetric(this IFloat64Tuple3D tuple, IFloat64Tuple3D maxTuple)
        {
            return Float64Vector3D.Create(tuple.X.ClampToSymmetric(maxTuple.X),
