...
namespace GeometricAlgebraFulcrumLib.MathBase.GeometricAlgebra.Restricted.Generic.LinearMaps.Rotors
{
        ...
        : RGaScaledRotorBase<T>
    {
        ...
        internal static RGaEuclideanScaledRotorSquared2D<T> Create(RGaProcessor<T> processor, Scalar<T> scalar0)
        {
            return new RGaEuclideanScaledRotorSquared2D<T>(processor, scalar0, processor.ScalarProcessor.CreateScalarZero());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static RGaEuclideanScaledRotorSquared2D<T> Create(RGaProcessor<T> processor, Scalar<T> scalar0, Scalar<T> scalar12)
        {
            return new RGaEuclideanScaledRotorSquared2D<T>(processor, scalar0, scalar12);
        }
