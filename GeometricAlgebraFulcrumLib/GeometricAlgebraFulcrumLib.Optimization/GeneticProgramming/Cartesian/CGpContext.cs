...
namespace GeometricAlgebraFulcrumLib.Optimization.GeneticProgramming.Cartesian
{
    ...
    {
        public static CGpContext Create(CGpParameters cgpParameters, CGpChromosomeEvaluator evaluator)
        {
            return new CGpContext(cgpParameters, evaluator);
        }

        public static CGpContext Create(CGpFunctionSet functionSet, int inputSize, int outputSize, int nodeRows, int nodeColumns, int arity, CGpChromosomeEvaluator evaluator)
        {
        ...
        public CGpChromosomeEvaluator Evaluator { get; }

        //public CGpFunctionSet FunctionSet
        //    => Parameters.FunctionSet;

        //public CGpRegressionDataSet? DataSet { get; set; }


        private CGpContext(CGpParameters cgpParameters, CGpChromosomeEvaluator evaluator)
        {
        ...
        public CGpChromosome RunCGp(int generationCount)
        {
            ...
            if (Evaluator is null)
                throw new InvalidOperationException("Chromosome Evaluator not present");
            ...
            /* set fitness of the parents */
            for (var i = 0; i < Parameters.Mu; i++) 
                parentChromosomes[i].UpdateCost(Evaluator);
            ...
            for (gen = 0; gen < generationCount; gen++)
            {
                /* set fitness of the children of the population */
                for (var i = 0; i < Parameters.Lambda; i++)
                    childChromosomes[i].UpdateCost(Evaluator);
