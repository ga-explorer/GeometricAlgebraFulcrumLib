...
public sealed class MetaExpressionToMatlabScalarProcessorConverter :
    MetaExpressionToLanguageConverterBase
{
    public static MetaExpressionToMatlabScalarProcessorConverter DefaultConverter { get; }
        = new MetaExpressionToMatlabScalarProcessorConverter("sp");

    public static MetaExpressionToMatlabScalarProcessorConverter Create(string scalarProcessorVariableName)
    {
        return new MetaExpressionToMatlabScalarProcessorConverter(scalarProcessorVariableName);
    }
    ...
    private MetaExpressionToMatlabScalarProcessorConverter(string scalarProcessorVariableName)
    ...
    public override SteExpression Visit(IMetaExpressionFunction functionExpr)
    {
        ...
        switch (functionName)
        {
            ...
            //{
            //    "-1" => SteExpression.CreateOperator(CclMatlabUtils.Operators.Divide,
            //        SteExpression.CreateLiteralNumber(1), argumentsArray[0]),
            //    "2" => SteExpression.CreateOperator(CclMatlabUtils.Operators.Multiply, argumentsArray[0],
            //        argumentsArray[0]),
            //    "3" => SteExpression.CreateOperator(CclMatlabUtils.Operators.Multiply, argumentsArray[0],
    ...
    private SteExpression Visit(int value)
    {
        return value switch
        {
            ...
            _ => SteExpression.CreateFunction(
                $"{ScalarProcessorVariableName}.ScalarFromNumber",
                value
            )
        };
    }
        ...
        return numberExpr.HeadSpecs switch
        {
            ...
            MetaExpressionHeadSpecsNumberRational rationalHeadSpecs =>
                SteExpression.CreateFunction(
                    $"{ScalarProcessorVariableName}.ScalarFromRational",
