    ...
    RGaFloat64Multivector
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public IReadOnlyList<RGaFloat64Vector> BladeToVectors(params int[] probeBasisVectorIndices)
    {
        var probeVectors = 
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public IReadOnlyList<RGaFloat64Vector> BladeToVectors(IEnumerable<int> probeBasisVectorIndices)
    {
        var probeVectors = 
            probeBasisVectorIndices
                .Select(index => Processor.CreateTermVector(index))
    ...
    public IReadOnlyList<RGaFloat64Vector> BladeToVectors(IReadOnlyList<RGaFloat64Vector> probeVectors)
    {
        if (IsZero || Grade == 0)
            return Array.Empty<RGaFloat64Vector>();

        if (this is RGaFloat64Vector vectorBlade)
            return new[] { vectorBlade };

        var vectorList = new List<RGaFloat64Vector>(Grade);
