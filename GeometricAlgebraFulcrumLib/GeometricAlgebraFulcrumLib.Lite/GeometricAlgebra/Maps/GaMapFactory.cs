...
using RGaBasisBladeComposerUtils = GeometricAlgebraFulcrumLib.Lite.GeometricAlgebra.Restricted.Basis.RGaBasisBladeComposerUtils;

// ReSharper disable CompareOfFloatsByEqualityOperator

namespace GeometricAlgebraFulcrumLib.Lite.GeometricAlgebra.Maps;
...
public static class GaMapFactory
{
    ...
    public static GaScaledPureRotor CreateScaledIdentityRotor(this RGaFloat64Processor basisSet, double scalingFactor)
    {
        ...
        var multivector = 
            basisSet.CreateScalar(Math.Sqrt(scalingFactor));
    ...
    public static GaScaledPureRotor CreatePureRotor(this RGaFloat64Multivector blade, IntegerSign bladeSignature)
    {
        ...
        {
            var mv = basisSet.CreateScalar(1);
        ...
        if (bladeSignature.IsNegative)
        {
            ...
            var mv = blade.Times(alpha.Sin() / alpha).Add(alpha.Cos());
        ...
        {
            var alpha = blade.SpSquared().Sqrt();
            var mv = blade.Times(alpha.Sinh() / alpha).Add(alpha.Cosh());
    ...
    /// <returns></returns>
    public static GaScaledPureRotor CreateEuclideanPureRotor(this RGaFloat64Bivector rotationBlade, Float64PlanarAngle rotationAngle)
    {
    ...
    public static GaScaledPureRotor CreateEuclideanParametricPureRotor3D(this RGaFloat64Processor basisSet, IFloat64Vector3D sourceVector, IFloat64Vector3D targetVector, Float64PlanarAngle angleTheta)
    {
        ...
        var pseudoScalarInverse =
            RGaBasisBladeComposerUtils.CreateBasisBlade(basisSet, 7).ToKVector().EInverse();
    ...
    /// <param name="rotationAngle"></param>
    /// <returns></returns>
    public static GaScaledPureRotor CreateGivensRotor(this RGaFloat64Processor basisSet, int i, int j, Float64PlanarAngle rotationAngle)
    {
        Debug.Assert(j != i);
        ...
        var bivectorPart = basisSet.CreateTermBivector(
            new Pair<int>(i, j),
