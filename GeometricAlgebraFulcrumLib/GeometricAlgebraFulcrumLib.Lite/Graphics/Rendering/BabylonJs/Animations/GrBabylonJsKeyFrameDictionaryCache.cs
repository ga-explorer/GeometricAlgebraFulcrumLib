...
public sealed class GrBabylonJsKeyFrameDictionaryCache
{
    ...
    public KeyFrameDictionaryCache<Float64Vector2D> Vector2KeyFramesCache { get; }
        = new KeyFrameDictionaryCache<Float64Vector2D>(GrBabylonJsAnimationType.Vector2);

    public KeyFrameDictionaryCache<Float64Vector3D> Vector3KeyFramesCache { get; }
        = new KeyFrameDictionaryCache<Float64Vector3D>(GrBabylonJsAnimationType.Vector3);
    
    public KeyFrameDictionaryCache<Float64Quaternion> QuaternionKeyFramesCache { get; }
        = new KeyFrameDictionaryCache<Float64Quaternion>(GrBabylonJsAnimationType.Quaternion);
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public KeyValuePair<int, GrBabylonJsKeyFrameDictionary<Float64Vector2D>> AddOrGetVector2KeyFrames(GrBabylonJsKeyFrameDictionary<Float64Vector2D> keyFrames)
    {
        return Vector2KeyFramesCache.AddOrGetKeyFrames(
            keyFrames,
            (v1, v2) => 
                (v1 - v2).ENormSquared().IsNearZero()
        );
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public KeyValuePair<int, GrBabylonJsKeyFrameDictionary<Float64Vector3D>> AddOrGetVector3KeyFrames(GrBabylonJsKeyFrameDictionary<Float64Vector3D> keyFrames)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public KeyValuePair<int, GrBabylonJsKeyFrameDictionary<Float64Quaternion>> AddOrGetQuaternionKeyFrames(GrBabylonJsKeyFrameDictionary<Float64Quaternion> keyFrames)
    {
        return QuaternionKeyFramesCache.AddOrGetKeyFrames(
            keyFrames,
            (v1, v2) => 
                (v1.ToRotationVector() - v2.ToRotationVector()).ENormSquared().IsNearZero()
        );
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public GrBabylonJsKeyFrameDictionary<Float64Vector2D> GetVector2KeyFrames(int index)
    {
        return Vector2KeyFramesCache[index];
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public GrBabylonJsKeyFrameDictionary<Float64Vector3D> GetVector3KeyFrames(int index)
    {
        return Vector3KeyFramesCache[index];
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public GrBabylonJsKeyFrameDictionary<Float64Quaternion> GetQuaternionKeyFrames(int index)
    {
