...
{
    public static GaVector<T> RotateUsingUnitVectors<T>(this GaVector<T> x, GaVector<T> u, GaVector<T> v)
    {
        var uvDot = u.Sp(v);
        var xuDot = x.Sp(u);
        var xvDot = x.Sp(v);
    ...
    public static void NumericExample1()
    {
        ...
        var u = random.GetVector(n, true);
        var v = random.GetVector(n, true);

        var uvRotor = u.GetEuclideanRotorTo(v);

        for (var i = 0; i < 100; i++)
        {
            var x = random.GetVector(n, false);
    ...
    public static void SymbolicExample1()
    {
        ...
        var y1 =
            w.CreatePureReflector().OmMap(x1);
    ...
    public static void Example2()
    {
        ...
        {
            var u = 
                geometricProcessor.CreateVectorBasis(m);
            ...
            var uvDot = $"Subscript[v, {m + 1}]".CreateScalar(geometricProcessor);
            var xuDot = $"Subscript[x, {m + 1}]".CreateScalar(geometricProcessor);
            var xvDot = x.Sp(v);

            Debug.Assert(
                (uvDot - u.Sp(v)).FullSimplify().IsZero()
            );
            
            Debug.Assert(
                (xuDot - x.Sp(u)).FullSimplify().IsZero()
            );
            
            Debug.Assert(
                (xvDot - x.Sp(v)).FullSimplify().IsZero()
            );
