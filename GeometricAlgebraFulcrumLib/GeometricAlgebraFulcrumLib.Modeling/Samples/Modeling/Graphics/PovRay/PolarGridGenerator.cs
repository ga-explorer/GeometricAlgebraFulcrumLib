...
public static class PolarGridGenerator
{
    public static string WorkingFolder { get; }
        = @"D:\Projects\Study\POV-Ray\Samples";

    public static Float64PolarGrid2D GridSpecs { get; }
        = Float64PolarGrid2D.Create(
            0.5, 
            12, 
            GridWidth, 
            GridHeight
    ...
    private static void AddGrid(LinFloat64Vector2D origin, int subDivisionLevel, LinFloat64Angle angle, Color color, GrPovRayFinish finish)
    {
        ...
        foreach (var lineSegment in gridSpecs.GetRadialLineSegments())
        {
        ...
        foreach (var radius in gridSpecs.GetCircleRadii())
        {
            Composer.AddStatement(
                GrPovRayObject
                    .Torus(
                    ...
                    .SetMaterial(color, finish)
                    .Translate(gridSpecs.Origin.ToZxLinVector3D())
            );
        }
    }
    ...
    {
        const string fileName = "PolarGridScene";
