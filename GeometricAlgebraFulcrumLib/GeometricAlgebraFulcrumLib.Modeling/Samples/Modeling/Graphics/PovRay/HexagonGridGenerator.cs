...
public static class HexagonGridGenerator
{
    public static string WorkingFolder { get; }
        = @"D:\Projects\Study\POV-Ray\Samples";

    public static Float64HexagonGrid2D GridSpecs { get; }
        = Float64HexagonGrid2D.Create(0.5, GridWidth, GridHeight);
    ...
    public static LinFloat64Vector3D SideVector3 
        => GridSpecs.BaseVectors.Item3.ToZxLinVector3D();
    ...
    ///// </summary>
    ///// <param name="cellSideObjName"></param>
    //private static void DefineCellSide(string cellSideObjName)
    //{
    //    Composer.SceneStatements.DeclareObject(
    //        cellSideObjName,
    //        GrPovRayObject.RegularPolygonCylinder(
    //            CellSideCount,
    //            CellSideRadius,
    //            0,
    //            CellRadius
    ...
    //            AffineMap3D
    //                .Create()
    //                .RotateY(LinFloat64PolarAngle.Angle90)
    //                .TranslateZ(CellSpecs.InnerRadius)
    //                .TranslateX(-0.5 * CellSpecs.SideLength)
    ...
    private static void DefineCell(string cellObjName, string cellSideObjName)
    {
        Composer.SceneStatements.DeclareObject(
            cellObjName,
            GrPovRayObject.Union(
                GrPovRayObject.Named(cellSideObjName),
                GrPovRayObject.Named(cellSideObjName).RotateY(LinFloat64PolarAngle.Angle120),
                GrPovRayObject.Named(cellSideObjName).RotateY(LinFloat64PolarAngle.Angle240)
            )
        );
    }
    ...
    private static void AddGrid(LinFloat64Vector2D origin, int subDivisionLevel, LinFloat64Angle angle, Color color, GrPovRayFinish finish)
    {
        ...
        foreach (var cellOrigin in gridSpecs)
        {
            var (x, y) = cellOrigin + gridSpecs.Origin;
    ...
    {
        const string fileName = "HexagonGridScene";
