...
public static class CatmullRomUtils
{
    ...
    /// <param name="t">the actual interpolation ratio from 0 to 1 representing the position between p1 and p2 to interpolate the value.</param>
    /// <returns>The interpolated value</returns>
    public static LinFloat64Vector2D GetCatmullRomValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector2D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
    ...
    public static LinFloat64Vector2D GetCatmullRomDerivativeValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector2D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => p.ToLinVector2D());
    ...
    public static LinFloat64Vector2D GetCatmullRomDerivative2Value(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector2D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => p.ToLinVector2D());
    ...
    /// <param name="t">the actual interpolation ratio from 0 to 1 representing the position between p1 and p2 to interpolate the value.</param>
    /// <returns>The interpolated value</returns>
    public static LinFloat64Vector3D GetCatmullRomValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector3D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => p.ToLinVector3D());
    ...
    public static LinFloat64Vector3D GetCatmullRomDerivativeValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector3D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => p.ToLinVector3D());
    ...
    public static LinFloat64Vector3D GetCatmullRomDerivative2Value(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector3D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => p.ToLinVector3D());
    ...
    /// <param name="t">the actual interpolation ratio from 0 to 1 representing the position between p1 and p2 to interpolate the value.</param>
    /// <returns>The interpolated value</returns>
    public static LinFloat64Vector4D GetCatmullRomValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector4D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => LinFloat64Vector4DUtils.ToLinVector4D(p));
    ...
    public static LinFloat64Vector4D GetCatmullRomDerivativeValue(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector4D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => LinFloat64Vector4DUtils.ToLinVector4D(p));
    ...
    public static LinFloat64Vector4D GetCatmullRomDerivative2Value(this double t, Quad<double> tQuad, Quad<ILinFloat64Vector4D> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) =
            pQuad.MapItems(p => LinFloat64Vector4DUtils.ToLinVector4D(p));
    ...
    /// <param name="t">the actual interpolation ratio from 0 to 1 representing the position between p1 and p2 to interpolate the value.</param>
    /// <returns>The interpolated value</returns>
    public static LinFloat64Vector GetCatmullRomValue(this double t, Quad<double> tQuad, Quad<LinFloat64Vector> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) = pQuad;
    ...
    public static LinFloat64Vector GetCatmullRomDerivativeValue(this double t, Quad<double> tQuad, Quad<LinFloat64Vector> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) = pQuad;
    ...
    public static LinFloat64Vector GetCatmullRomDerivative2Value(this double t, Quad<double> tQuad, Quad<LinFloat64Vector> pQuad)
    {
        var (t0, t1, t2, t3) = tQuad;
        var (p0, p1, p2, p3) = pQuad;
