    ...
    IParametricCurveSampler2D
{
    ...
    public UniformParameterCurveSampler2D(IFloat64ParametricCurve2D curve, Float64ScalarRange parameterRange, int count, bool isPeriodic = false)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public UniformParameterCurveSampler2D SetCurve(IFloat64ParametricCurve2D curve, Float64ScalarRange parameterRange, int count, bool isPeriodic)
    {
        if (isPeriodic && count < 1 || !isPeriodic && count < 2)
            throw new ArgumentOutOfRangeException(nameof(count));
