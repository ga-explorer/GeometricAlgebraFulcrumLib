    ...
    IParametricC2Curve2D
{
    ...
    internal CatmullRomSpline2D(IEnumerable<ILinFloat64Vector2D> inputPointList, CatmullRomSplineType curveType, bool isClosed)
    {
    ...
    public LinFloat64Vector2D GetDerivative1Point(double parameterValue)
    {
        if (parameterValue is <= 0d or >= 1d)
            return LinFloat64Vector2D.Create((Float64Scalar)Differentiate.FirstDerivative(GetPointX, parameterValue),
    ...
    {
        if (parameterValue is <= 0d or >= 1d)
            return LinFloat64Vector2D.Create((Float64Scalar)Differentiate.SecondDerivative(GetPointX, parameterValue),
    ...
    {
        return GetDerivative1Point(parameterValue).ToUnitLinVector2D();
    }
