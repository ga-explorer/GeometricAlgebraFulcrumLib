...
public sealed class XGaGeometrySpaceBasisSpecs<T>
{
    ...
    public static XGaGeometrySpaceBasisSpecs<T> CreateEGa(IScalarProcessor<T> scalarProcessor, int vSpaceDimensions)
    {
        ...
        return new XGaGeometrySpaceBasisSpecs<T>(
            XGaConformalProcessor<T>.CreateEuclidean(scalarProcessor), 
            laTeXVectorSubscripts
        );
    }

    public static XGaGeometrySpaceBasisSpecs<T> CreateCGa(IScalarProcessor<T> scalarProcessor, int vSpaceDimensions)
    {
    ...
    public static XGaGeometrySpaceBasisSpecs<T> CreateCGaOrthogonal(IScalarProcessor<T> scalarProcessor, int vSpaceDimensions)
    {
        if (vSpaceDimensions < 4)
            throw new ArgumentOutOfRangeException(nameof(vSpaceDimensions));
    ...
    private XGaGeometrySpaceBasisSpecs(XGaProcessor<T> processor, IReadOnlyList<string> laTeXVectorSubscripts)
    {
        Processor = processor;
    ...
    private XGaGeometrySpaceBasisSpecs(XGaProcessor<T> processor, IReadOnlyList<string> laTeXVectorSubscripts, IXGaOutermorphism<T> basisMap, IXGaOutermorphism<T> basisMapInverse)
    {
        Processor = processor;
        EuclideanProcessor = processor as XGaEuclideanProcessor<T> ?? processor.ScalarProcessor.CreateEuclideanXGaProcessor();
        LaTeXVectorSubscripts = laTeXVectorSubscripts;
