...
{
    public sealed class CGaOpnsFlatEncoder<T> :
        CGaEncoderBase<T>
    {
        internal CGaOpnsFlatEncoder(CGaGeometricSpace<T> geometricSpace)
            : base(geometricSpace)
        {
        }
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public CGaBlade<T> Line(CGaBlade<T> egaPoint, CGaBlade<T> egaDirection)
        {
            Debug.Assert(
