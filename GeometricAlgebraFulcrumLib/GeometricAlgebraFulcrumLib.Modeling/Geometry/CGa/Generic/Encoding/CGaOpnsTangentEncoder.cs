...
public sealed class CGaOpnsTangentEncoder<T> :
    CGaEncoderBase<T>
{
    internal CGaOpnsTangentEncoder(CGaGeometricSpace<T> geometricSpace)
        : base(geometricSpace)
    {
    }
    ...
    {
        return GeometricSpace.Eo.TranslateBy(egaPoint);
    ...
    {
        var direction =
            egaDirection.EncodeVGaBivector(GeometricSpace)/*.DivideByNorm()*/;
    ...
    {
        var direction =
            egaDirection.EncodeVGaBlade(GeometricSpace)/*.DivideByNorm()*/;
