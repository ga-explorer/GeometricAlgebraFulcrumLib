...
public sealed class GrBabylonJsSnapshotSpecs
{
    ...
    public string GetSnapshotCode(string sceneName)
    {
        ...
        var sizeCode = UsePrecision
            ? $"{{ precision: {Precision.GetAttributeValueCode()} }}"
            : $"{{ width: {Width.GetAttributeValueCode()}, height: {Height.GetAttributeValueCode()} }}";
...
function saveScreenShot(scene, delay, fileName, size) {
	...
	BABYLON.Tools.DelayAsync(delay).then(() => { 
		BABYLON.Tools.CreateScreenshot(
            ...
            .AppendLine(@$"    saveScreenShot({sceneName}, {Delay.GetAttributeValueCode()}, '{FileName}', {sizeCode});")
            .AppendLine(@"});")
            .AppendLine();
