...
using GeometricAlgebraFulcrumLib.Matlab.Structures.Permutations;
    ...
    IXGaFloat64VectorFrame
{
    ...
    public XGaFloat64VectorFrame GetOrthonormalFrame()
    {
        ...
        {
            orthogonalVector.Divide(orthogonalVector.Norm())
        };
    ...
    public XGaFloat64VectorFrame GetSwappedPairsFrame()
    {
        ...
        if (_vectorList.Count % 2 == 1)
        {
            ...
            var lastVector = numberOfSwaps % 2 == 0
                ? _vectorList[_vectorList.Count - 1]
                : _vectorList[_vectorList.Count - 1].Negative();
    ...
    public double[,] GetArray(int rowsCount)
    {
        ...
        for (var j = 0; j < Count; j++)
        {
            ...
            foreach (var (index, scalar) in vectorTerms.ToTuples())
    ...
    public double[,] GetInnerProductsArray()
    {
        ...
        for (var i = 0; i < Count; i++)
        {
            ...
            ipm[i, i] = v1.SpSquared();

            for (var j = i + 1; j < Count; j++)
            {
                var ip = v1.Sp(_vectorList[j]);
