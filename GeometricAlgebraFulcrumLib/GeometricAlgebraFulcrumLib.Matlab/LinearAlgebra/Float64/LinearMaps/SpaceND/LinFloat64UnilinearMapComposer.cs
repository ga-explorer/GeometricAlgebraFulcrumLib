    ...
    IReadOnlyDictionary<IndexPair, double>
{
    ...
    {
        foreach (var (index1, scalar) in termList.ToTuples())
    ...
    {
        foreach (var (basisVector, scalar) in termList.ToTuples())
    ...
    public LinFloat64UnilinearMapComposer SetColumnVector(int index2, LinFloat64Vector vector)
    {
    ...
    public LinFloat64UnilinearMapComposer SetColumnVectorNegative(int index2, LinFloat64Vector vector)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            SetTerm(index1, index2, -scalar);
    ...
    public LinFloat64UnilinearMapComposer SetColumnVector(int index2, LinFloat64Vector vector, double scalingFactor)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
    ...
    public LinFloat64UnilinearMapComposer SetDiagonalMap(LinFloat64Vector vector)
    {
        _indexPairScalarDictionary = new Dictionary<IndexPair, double>();

        foreach (var (i, scalar) in vector.ToTuples())
        {
            _indexPairScalarDictionary.Add(
                new IndexPair(i),
                scalar
    ...
    {
        foreach (var ((index1, index2), scalar) in map.ToTuples())
            SetTerm(index1, index2, scalar);
    ...
    public LinFloat64UnilinearMapComposer AddColumnVector(int index2, LinFloat64Vector vector)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            AddTerm(index1, index2, scalar);
    ...
    public LinFloat64UnilinearMapComposer AddColumnVector(int index2, LinFloat64Vector vector, double scalingFactor)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            AddTerm(
                index1,
    ...
    public LinFloat64UnilinearMapComposer AddColumnVectorNegative(int index2, LinFloat64Vector vector)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            AddTerm(
                index1,
    ...
    public LinFloat64UnilinearMapComposer AddDiagonalMap(LinFloat64Vector vector)
    {
        foreach (var (i, scalar) in vector.ToTuples())
            AddTerm(i, i, scalar);
    ...
    public LinFloat64UnilinearMapComposer SubtractColumnVector(int index2, LinFloat64Vector vector)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            SubtractTerm(index1, index2, scalar);
    ...
    public LinFloat64UnilinearMapComposer SubtractColumnVector(int index2, LinFloat64Vector vector, double scalingFactor)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            SubtractTerm(
                index1,
    ...
    public LinFloat64UnilinearMapComposer SubtractColumnVectorNegative(int index2, LinFloat64Vector vector)
    {
        foreach (var (index1, scalar) in vector.ToTuples())
            SubtractTerm(
                index1,
    ...
    public LinFloat64UnilinearMapComposer SubtractDiagonalMap(LinFloat64Vector vector)
    {
        foreach (var (i, scalar) in vector.ToTuples())
            SubtractTerm(i, i, scalar);
    ...
    public LinFloat64UnilinearMapComposer MapScalars(Func<double, double> mappingFunction)
    {
        ...
        foreach (var (id, scalar) in _indexPairScalarDictionary.ToTuples())
        {
    ...
    public LinFloat64UnilinearMapComposer MapTerms(Func<int, int, double, KeyValuePair<IndexPair, double>> mappingFunction)
    {
        ...
        foreach (var (id, scalar) in _indexPairScalarDictionary.ToTuples())
        {
            var (termIndex, termScalar) = mappingFunction(id.Index1, id.Index2, scalar).ToTuple();
    ...
    public LinFloat64UnilinearMapComposer MapTerms(Func<IndexPair, double, KeyValuePair<IndexPair, double>> mappingFunction)
    {
        ...
        foreach (var (id, scalar) in _indexPairScalarDictionary.ToTuples())
        {
            var (termIndex, termScalar) = mappingFunction(id, scalar).ToTuple();
    ...
    public LinFloat64UnilinearMapComposer Times(LinFloat64UnilinearMap map2)
    {
        ...
        foreach (var (index2, vector) in map2.GetMappedBasisVectors().ToTuples())
        {
            var vector1 = GetMappedColumnVector(vector);

            foreach (var (index1, scalar) in vector1.ToTuples())
    ...
    public LinFloat64Vector GetMappedBasisVector(int index)
    {
        var composer = LinFloat64VectorComposer.Create();

        foreach (var ((index1, index2), scalar1) in _indexPairScalarDictionary.ToTuples())
        {
    ...
    public LinFloat64Vector GetMappedColumnVector(IReadOnlyList<double> vector)
    {
        var composer = LinFloat64VectorComposer.Create();
    ...
    public LinFloat64Vector GetMappedColumnVector(LinFloat64Vector vector)
    {
        var composer = LinFloat64VectorComposer.Create();
    ...
    public double[,] GetMapArray(int rowCount, int colCount)
    {
        ...
        foreach (var ((rowIndex, colIndex), scalar) in _indexPairScalarDictionary.ToTuples())
        {
            if (!scalar.IsValid())
                throw new InvalidOperationException();
