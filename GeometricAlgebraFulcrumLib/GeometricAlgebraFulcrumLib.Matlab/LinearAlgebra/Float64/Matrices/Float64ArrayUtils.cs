...
public static class Float64ArrayUtils
{
    ...
    public static ScalarArrayRow<double> GetRow(this double[,] scalarArray, int rowIndex)
    {
        return new ScalarArrayRow<double>(scalarArray, rowIndex);
    }

    
    public static ScalarArrayColumn<double> GetColumn(this double[,] scalarArray, int colIndex)
    {
        return new ScalarArrayColumn<double>(scalarArray, colIndex);
    }

    
    public static ScalarArrayDiagonal<double> GetDiagonal(this double[,] scalarArray)
    {
        return new ScalarArrayDiagonal<double>(scalarArray);
    }

    
    public static IEnumerable<ScalarArrayRow<double>> GetRows(this double[,] scalarArray)
    {
        return scalarArray
            .GetLength(1)
            .GetRange(i =>
                new ScalarArrayRow<double>(scalarArray, i)
            );
    }

    
    public static IEnumerable<ScalarArrayColumn<double>> GetColumns(this double[,] scalarArray)
    {
        return scalarArray
            .GetLength(1)
            .GetRange(i =>
                new ScalarArrayColumn<double>(scalarArray, i)
            );
    }
    ...
    {
        return vector.Prepend(double.PositiveInfinity).Min();
    }

    
    public static double GetMax(this IEnumerable<double> vector)
    {
        return vector.Prepend(double.NegativeInfinity).Max();
    }
    ...
    public static double VectorAngleCos(this IReadOnlyList<double> u, IReadOnlyList<double> v)
    {
        ...
        return (uvDot / (uuDot * vvDot).Sqrt()).Clamp(-1, 1);
    }
    ...
    public static double[] CreateZero1D(int size)
    {
    ...
    public static double[,] CreateZero(int size)
    {
    ...
    public static double[,] CreateZero(int rowCount, int colCount)
    {
    ...
    private static double[,] CreateClarkeRotationArrayOdd(int size)
    {
        ...
        for (var k = 0; k < n; k++)
        {
            ...
            {
                var angle = 2 * Math.PI * (k + 1) * colIndex / size;
                // $"2 * Pi * {k + 1} * {i} / {m}";
    ...
    public static LinFloat64Vector MatrixProduct(this double[,] matrix, IReadOnlyDictionary<int, double> vector)
    {
        ...
        foreach (var (j, scalar) in vector.ToTuples())
        {
            if (j >= n) continue;
    ...
    public static double[,] MatrixProduct(this double[,] scalarsArray1, params double[][,] scalarsArrays)
    {
        return scalarsArrays.Aggregate(
            scalarsArray1,
            (a, b) => a.MatrixProduct(b)
        );
    }
