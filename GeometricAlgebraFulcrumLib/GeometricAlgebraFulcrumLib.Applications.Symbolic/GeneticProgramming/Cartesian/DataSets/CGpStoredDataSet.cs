...
public class CGpStoredDataSet :
    CGpDataSet
{
    public static CGpStoredDataSet Create(int inputSize, int outputSize)
    {
        return new CGpStoredDataSet(inputSize, outputSize);
    }

    public static CGpStoredDataSet CreateFromFile(string filePath)
    {
        var textLines = 
        ...
        var dataSet = new CGpStoredDataSet(inputSize, outputSize);
    ...
    private readonly List<CGpDataSetSample> _sampleList
        = new List<CGpDataSetSample>();

    public override int SampleCount
        => _sampleList.Count;


    private CGpStoredDataSet(int inputSize, int outputSize)
        : base(inputSize, outputSize)
    {
    }


    public CGpStoredDataSet Clear()
    {
        _sampleList.Clear();
    ...
    public CGpStoredDataSet SetSample(int sampleIndex, IReadOnlyList<double> sampleInput, IReadOnlyList<double> sampleOutput)
    {
        ...
        _sampleList[sampleIndex] = new CGpDataSetSample(sampleInput, sampleOutput);

        return this;
    }

    public CGpStoredDataSet AddSample(IReadOnlyList<double> sampleInput, IReadOnlyList<double> sampleOutput)
    {
        if (sampleInput.Count != InputSize)
            throw new ArgumentException(nameof(sampleInput));
        ...
        _sampleList.Add(
            new CGpDataSetSample(sampleInput, sampleOutput)
        );
    ...
    public override IEnumerator<CGpDataSetSample> GetEnumerator()
    {
