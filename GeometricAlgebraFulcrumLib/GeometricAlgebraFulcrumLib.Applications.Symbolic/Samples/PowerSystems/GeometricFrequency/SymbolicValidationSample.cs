...
namespace GeometricAlgebraFulcrumLib.Applications.Symbolic.Samples.PowerSystems.GeometricFrequency;
...
public static class SymbolicValidationSample
{
    ...
    private static RGaVector<Expr> DefineCurve1(Expr tExpr)
    {
        ...
        var a1 = GeometricProcessor.Vector(1, 2, 3);
        var a2 = GeometricProcessor.Vector(-1, -1, 0);
        var a3 = GeometricProcessor.Vector(0, 1, 1);

        var b1 = GeometricProcessor.Vector(-2, 1, 1);
        var b2 = GeometricProcessor.Vector(-1, 0, 0);
        var b3 = GeometricProcessor.Vector(1, 2, 2);
    ...
    private static RGaVector<Expr> DefineCurve2(Expr tExpr)
    {
        ...
        var a1 = GeometricProcessor.Vector(1, 2, 3, 4);
        var a2 = GeometricProcessor.Vector(-1, -1, 0, 1);
        var a3 = GeometricProcessor.Vector(0, 1, 1, 2);

        var b1 = GeometricProcessor.Vector(-2, 1, 1, 2);
        var b2 = GeometricProcessor.Vector(-1, 0, 0, -1);
        var b3 = GeometricProcessor.Vector(1, 2, 2, -2);
    ...
    private static RGaVector<Expr> DefineCurve3(Expr tExpr)
    {
        ...
        var a2 = GeometricProcessor.Vector(-2, 1, 1);
        var a3 = GeometricProcessor.Vector(-2, 1, 2);
        //var a4 = GeometricProcessor.Vector(-1, 2, 1);
        //var a5 = GeometricProcessor.Vector(1, 2, 2);
    ...
    private static RGaVector<Expr> DefineCurve4(Expr tExpr)
    {
        ...
        var a2 = GeometricProcessor.Vector(-2, 1, 1, 2);
        var a3 = GeometricProcessor.Vector(-2, 1, 2, -1);
        var a4 = GeometricProcessor.Vector(-1, 2, 1, -1);
        //var a5 = GeometricProcessor.Vector(1, 2, 2, -2);
    ...
    private static RGaVector<Expr> DefineCurve5(Expr tExpr)
    {
        ...
        var a2 = GeometricProcessor.Vector(0, 1, 0);
        var a3 = GeometricProcessor.Vector(0, 0, 1);
    ...
    private static RGaVector<Expr> DefineCurve6(Expr tExpr)
    {
        ...
        var a2 = GeometricProcessor.Vector(0, 1, 0, 0);
        var a3 = GeometricProcessor.Vector(0, 0, 1, 0);
        var a4 = GeometricProcessor.Vector(0, 0, 0, 1);
    ...
    private static RGaVector<Expr> DefineCurve7(Expr tExpr)
    {
        var aArray = new[]
        {
            GeometricProcessor.Vector(0, 0, 0, 0, 0, 0),
            GeometricProcessor.Vector(1, 0, 0, 0, 0, 0),
            GeometricProcessor.Vector(0, 1, 0, 0, 0, 0),
            GeometricProcessor.Vector(0, 0, 1, 0, 0, 0),
            GeometricProcessor.Vector(0, 0, 0, 1, 0, 0),
            GeometricProcessor.Vector(0, 0, 0, 0, 1, 0),
            GeometricProcessor.Vector(0, 0, 0, 0, 0, 1)
        };
    ...
    private static RGaVector<Expr> DefineCurve8(Expr tExpr)
    {
        ...
        var a = GeometricProcessor.Vector(1, 2, 3);
        var b = GeometricProcessor.Vector(-2, 1, 1);
    ...
    private static RGaVector<Expr> DefineCurve9(Expr tExpr)
    {
        ...
        var a = GeometricProcessor.Vector(1, 2, 3, 4);
        var b = GeometricProcessor.Vector(-2, 1, 1, 2);
    ...
    private static RGaVector<Expr> DefineCurve10(Expr tExpr)
    {
        ...
        return GeometricProcessor.Vector(scalarArray).SimplifyScalars();
    }
    ...
    private static void Plot(Scalar<Expr> curve, string filePath)
    {
        Plot(curve.ScalarValue, filePath);
    }
    
    private static void Plot(IScalar<Expr> curve, string filePath)
    {
        Plot(curve.ScalarValue, filePath);
    }
    ...
    public static void Example3D()
    {
        ...
        var (e1sDs, e2sDs, e3sDs) =
            GetFrameArcLengthDerivative3D(e1s, e2s, e3s, sDt1.ScalarValue);
        ...
        var scalarZero = Expr.INT_ZERO.ScalarFromValue(ScalarProcessor);

        var kappa1_1 = u2sNorm.IsZero() ? scalarZero : (u2sNorm / u1sNorm).ToScalar();
        var kappa2_1 = u3sNorm.IsZero() ? scalarZero : (u3sNorm / u2sNorm).ToScalar();
    ...
    public static void Example4D()
    {
        ...
        var (e1sDs, e2sDs, e3sDs, e4sDs) =
            GetFrameArcLengthDerivative4D(e1s, e2s, e3s, e4s, sDt1.ScalarValue);
        ...
        var kappa1_1 = u2sNorm.IsZero() ? scalarZero : (u2sNorm / u1sNorm).ToScalar();
        var kappa2_1 = u3sNorm.IsZero() ? scalarZero : (u3sNorm / u2sNorm).ToScalar();
        var kappa3_1 = u4sNorm.IsZero() ? scalarZero : (u4sNorm / u3sNorm).ToScalar();
    ...
    public static void Example6D()
    {
        ...
        var (e1sDs, e2sDs, e3sDs, e4sDs, e5sDs, e6sDs) =
            GetFrameArcLengthDerivative6D(e1s, e2s, e3s, e4s, e5s, e6s, sDt1.ScalarValue);
        ...
        var kappa2_1 = u3sNorm.IsZero() ? scalarZero : (u3sNorm / u2sNorm).ToScalar();
        var kappa3_1 = u4sNorm.IsZero() ? scalarZero : (u4sNorm / u3sNorm).ToScalar();
        var kappa4_1 = u5sNorm.IsZero() ? scalarZero : (u5sNorm / u4sNorm).ToScalar();
        var kappa5_1 = u6sNorm.IsZero() ? scalarZero : (u6sNorm / u5sNorm).ToScalar();
