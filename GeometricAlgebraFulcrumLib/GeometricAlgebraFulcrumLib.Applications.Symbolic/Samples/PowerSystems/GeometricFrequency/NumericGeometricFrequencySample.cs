...
public static class NumericGeometricFrequencySample
{
    ...
    private static XGaVector<Float64Signal> InitializeSyntheticSignal3()
    {
        ...
        var w = @"\[Omega]".ScalarFromText(scalarProcessor); //"2 * Pi * 50".CreateScalar(ScalarProcessor);
        var theta = "2 * Pi / 3".ScalarFromText(scalarProcessor);

        var v1 = "200 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (w * t).Sin(),
            (w * t - theta).Sin(),
            (w * t + theta).Sin()
        );

        var v2 = "20 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (2 * (w * t)).Sin(),
            (2 * (w * t - theta)).Sin(),
            (2 * (w * t + theta)).Sin()
        );

        var v3 = "-30 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (7 * (w * t)).Sin(),
            (7 * (w * t - theta)).Sin(),
            (7 * (w * t + theta)).Sin()
        );
        ...
        var s1 = v.MapScalars(s =>
            s.ReplaceAll(w.ScalarValue, freq.ToExpr())
        );

        return s1.GetSampledSignal(GeometricSignalProcessor, t.ScalarValue, samplingRate, sampleCount);
    }
    ...
    public static void SymbolicNumericValidationExample1()
    {
        ...
        var tMinNum = 0d;
        var tMaxNum = 2d;
        var tMinSym = scalarProcessor.ScalarFromNumber(tMinNum);
        var tMaxSym = scalarProcessor.ScalarFromNumber(tMaxNum);
    ...
    public static void SymbolicNumericValidationExample2()
    {
        ...
        var symV1 = "200 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (w * t).Sin(),
            (w * t - theta).Sin(),
            (w * t + theta).Sin()
        );

        var symV2 = "20 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (2 * (w * t)).Sin(),
            (2 * (w * t - theta)).Sin(),
            (2 * (w * t + theta)).Sin()
        );

        var symV3 = "-30 * Sqrt[2]".ToExpr() * geometricProcessor.Vector(
            (7 * (w * t)).Sin(),
            (7 * (w * t - theta)).Sin(),
            (7 * (w * t + theta)).Sin()
        );
        ...
        var omega1Mean = omega1.MapScalars(s =>
            s.IntegrateScalar(t.ScalarValue, tMin.ScalarValue, tMax.ScalarValue).TrigReduce()
        );
