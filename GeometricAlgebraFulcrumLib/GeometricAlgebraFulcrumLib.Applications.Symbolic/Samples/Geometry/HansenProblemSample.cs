...
namespace GeometricAlgebraFulcrumLib.Applications.Symbolic.Samples.Geometry;
...
public static class HansenProblemSample
{
    ...
    public static void GenerateTrigCode()
    {
        ...
        var s0 = (1 - tanLambda) / (1 + tanLambda);
        ...
        var gamma = (s2 + s1).HalfPolarAngle();
        var delta = (s2 - s1).HalfPolarAngle();
        
        var epsilon = 
            gamma - (alpha1M + alpha2M + beta1M + beta2M);
        ...
        //var pvP1 = pvA + ap1 * v1;
        //var pvP2 = pvA + ap2 * v2;
        var pvP1 = xp1 * e1 + yp1 * e2;
        var pvP2 = xp2 * e1 + yp2 * e2;
    ...
    public static void GenerateCGaCode()
    {
        ...
        var a =
            cgaSpace.EncodeIpnsRoundPoint(xa, ya);

        var b =
            cgaSpace.EncodeIpnsRoundPoint(xb, yb);

        // Encode line passing through A,B as CGA flat line blade l
        var l =
            cgaSpace.EncodeIpnsFlatLineFromPoints(pvA, pvB);

        var bvA = 
            cgaSpace.EncodeIpnsFlatPoint(pvA).InternalBivector;
        
        var bvB = 
            cgaSpace.EncodeIpnsFlatPoint(pvB).InternalBivector;
        ...
        var rA1 = 
            (halfAlpha1Cos + halfAlpha1Sin / bvA.Norm() * bvA).ToConformalCGaVersor(cgaSpace);
        ...
        var rB1 = 
            (halfBeta1Cos + halfBeta1Sin / bvB.Norm() * bvB).ToConformalCGaVersor(cgaSpace);
        ...
        var rB2 = 
            (halfAlpha2Cos + halfAlpha2Sin / bvB.Norm() * bvB).ToConformalCGaVersor(cgaSpace);
        ...
        var rA2 = 
            (halfBeta2Cos + halfBeta2Sin / bvA.Norm() * bvA).ToConformalCGaVersor(cgaSpace);
