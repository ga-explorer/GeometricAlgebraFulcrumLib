...
namespace GeometricAlgebraFulcrumLib.Processing.Matrices.Float64
{
        ...
        : IGaMatrixProcessor<Matrix, double>
    {
        ...
        public IGaListEven<double> MatrixRowToVector(Matrix matrix, int rowIndex)
        {
        ...
        public IGaListEven<double> MatrixColumnToVector(Matrix matrix, int colIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IGaGridEven<double> MatrixToArray(Matrix matrix)
        {
            return matrix.AsArray().CreateEvenGridDense();
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Matrix CreateMatrix(IGaGridEven<double> array)
        {
            return (Matrix) Matrix.Build.DenseOfArray(array.ToArray());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Matrix CreateRowVectorMatrix(IGaListEven<double> array)
        {
        ...
        public Matrix CreateRowVectorMatrix(IGaGridEven<double> array, int rowIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Matrix CreateColumnVectorMatrix(IGaListEven<double> array)
        {
            return (Matrix) Matrix.Build.DenseOfColumnMajor(
                array.GetSparseCount(),
                1,
                array.ToArray(GetZeroScalar)
            );
        }

        public Matrix CreateColumnVectorMatrix(IGaGridEven<double> array, int columnIndex)
        {
        ...
        public int MatrixEigenDecomposition(Matrix matrix, out Tuple<double, IGaListEven<double>>[] realPairs, out Tuple<double, IGaListEven<double>>[] imagPairs)
        {
            var sysExpr = matrix.ToComplex().Evd();

            var count = sysExpr.EigenValues.Count;

            realPairs = new Tuple<double, IGaListEven<double>>[count];
            imagPairs = new Tuple<double, IGaListEven<double>>[count];
            ...
            for (var j = 0; j < count; j++)
            {
                ...
                realPairs[j] = new Tuple<double, IGaListEven<double>>(
                    complexEigenValue.Real,
                    complexEigenVector.Real().CreateEvenListDenseArray()
                );

                imagPairs[j] = new Tuple<double, IGaListEven<double>>(
                    complexEigenValue.Imaginary,
                    complexEigenVector.Imaginary().CreateEvenListDenseArray()
                );
            }
        ...
        public Pair<IGaListEven<double>>[] MatrixEigenVectors(Matrix matrix)
        {
            var sysExpr = matrix.ToComplex().Evd();
            var count = sysExpr.EigenValues.Count;
            var eigenVectorTuples = new Pair<IGaListEven<double>>[count];
            ...
            for (var j = 0; j < count; j++)
            {
                ...
                eigenVectorTuples[j] = new Pair<IGaListEven<double>>(
                    complexEigenVector.Real().CreateEvenListDenseArray(),
                    complexEigenVector.Imaginary().CreateEvenListDenseArray()
                );
            }
