...
namespace GeometricAlgebraFulcrumLib.Processing.Products.Iterators
{
    ...
    {
        public static IEnumerable<Tuple<T, IGasKVector<T>>> GetGbtOutermorphismScaledKVectors<T>(this GaMultivector<T> mv, IReadOnlyList<IGasVector<T>> basisVectorMappings)
        {
            return GaGbtMultivectorOutermorphismStack<T>.Create(basisVectorMappings, mv)
                .TraverseForScaledKVectors();
        }

        public static IEnumerable<GaTerm<T>> GetGbtOutermorphismIdScalarPairs<T>(this GaMultivector<T> mv, IReadOnlyList<IGasVector<T>> basisVectorMappings)
        {
