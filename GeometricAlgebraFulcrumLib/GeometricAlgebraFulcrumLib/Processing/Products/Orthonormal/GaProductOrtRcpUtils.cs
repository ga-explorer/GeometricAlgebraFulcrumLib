...
namespace GeometricAlgebraFulcrumLib.Processing.Products.Orthonormal
{
    public static class GaProductOrtRcpUtils
    {
        ...
        public static IGasScalar<T> Rcp<T>(this IGaSignature signature, IGasScalar<T> mv1, IGasScalar<T> mv2)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasScalar<T> Rcp<T>(this IGaSignature signature, IGasVectorTerm<T> mv1, IGasVectorTerm<T> mv2)
        {
            if (mv1.Index != mv2.Index)
                return mv1.ScalarProcessor.CreateZeroScalar();

            var sig = signature.RcpSignature(
                1UL << (int) mv1.Index, 
                1UL << (int) mv2.Index
            );
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static IGasKVectorTerm<T> RcpAsKVectorTerm<T>(this IGaSignature signature, IGasKVectorTerm<T> mv1, IGasKVectorTerm<T> mv2)
        {
            ...
            var sig = 
                signature.RcpSignature(mv1.Id, mv2.Id);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasKVectorTerm<T> Rcp<T>(this IGaSignature signature, IGasKVectorTerm<T> mv1, IGasKVectorTerm<T> mv2)
        {
            return mv1 switch
            {
                ...
                IGasVectorTerm<T> vt1 when mv2 is IGasVectorTerm<T> vt2 => 
                    Rcp(signature, vt1, vt2),

                _ => 
                    RcpAsKVectorTerm(signature, mv1, mv2)
            };
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasScalar<T> VectorsRcp<T>(this IGaSignature signature, IGaScalarProcessor<T> scalarProcessor, IReadOnlyList<T> vector1, IReadOnlyList<T> vector2)
        {
        ...
        private static IGasScalar<T> RcpAsScalar<T>(this IGaSignature signature, IGasVector<T> mv1, IGasVector<T> mv2)
        {
            var scalarProcessor = mv1.ScalarProcessor;
        ...
        public static IGasScalar<T> Rcp<T>(this IGaSignature signature, IGasVector<T> mv1, IGasVector<T> mv2)
        {
            return mv1 is IGasVectorTerm<T> vt1 && mv2 is IGasVectorTerm<T> vt2
                ? Rcp(signature, vt1, vt2)
                : RcpAsScalar(signature, mv1, mv2);
        }

        private static IGasKVector<T> RcpAsKVector<T>(this IGaSignature signature, IGasKVector<T> mv1, IGasKVector<T> mv2)
        {
            var scalarProcessor = 
                mv1.ScalarProcessor;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasKVector<T> Rcp<T>(this IGaSignature signature, IGasKVector<T> mv1, IGasKVector<T> mv2)
        {
            if (mv2.Grade > mv1.Grade)
                return mv1.ScalarProcessor.CreateZeroScalar();
            ...
            return mv1 switch
            {
                ...
                IGasKVectorTerm<T> kvt1 when mv2 is IGasKVectorTerm<T> kvt2 => 
                    RcpAsKVectorTerm(signature, kvt1, kvt2),

                IGasVector<T> v1 when mv2 is IGasVector<T> v2 => 
                    RcpAsScalar(signature, v1, v2),

                _ => 
                    RcpAsKVector(signature, mv1, mv2)
            };
        }

        private static IGasGradedMultivector<T> RcpAsGaMultivectorGraded<T>(this IGaSignature signature, IGasGradedMultivector<T> mv1, IGasGradedMultivector<T> mv2)
        {
            var scalarProcessor = 
                mv1.ScalarProcessor;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasGradedMultivector<T> Rcp<T>(this IGaSignature signature, IGasGradedMultivector<T> mv1, IGasGradedMultivector<T> mv2)
        {
            return mv1 switch
            {
                IGasScalar<T> s1 when mv2 is IGasScalar<T> s2 => 
                ...
                IGasKVector<T> kv1 when mv2 is IGasKVector<T> kv2 => 
                    RcpAsKVector(signature, kv1, kv2),

                _ => 
                    RcpAsGaMultivectorGraded(signature, mv1, mv2)
            };
        }

        private static IGasTermsMultivector<T> RcpAsTermsMultivector<T>(this IGaSignature signature, IGasMultivector<T> mv1, IGasMultivector<T> mv2)
        {
            var scalarProcessor = 
                mv1.ScalarProcessor;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasTermsMultivector<T> Rcp<T>(this IGaSignature signature, IGasTermsMultivector<T> mv1, IGasTermsMultivector<T> mv2)
        {
            return mv1 switch
            {
                IGasScalar<T> s1 when mv2 is IGasScalar<T> s2 => 
                ...
                _ => 
                    RcpAsTermsMultivector(signature, mv1, mv2)
            };
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasMultivector<T> Rcp<T>(this IGaSignature signature, IGasMultivector<T> mv1, IGasMultivector<T> mv2)
        {
            return mv1 switch
            {
                IGasScalar<T> s1 when mv2 is IGasScalar<T> s2 => 
                ...
                IGasGradedMultivector<T> gmv1 when mv2 is IGasGradedMultivector<T> gmv2 => 
                    RcpAsGaMultivectorGraded(signature, gmv1, gmv2),
