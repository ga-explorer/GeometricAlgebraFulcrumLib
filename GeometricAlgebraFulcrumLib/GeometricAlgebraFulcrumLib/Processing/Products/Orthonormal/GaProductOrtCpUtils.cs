...
namespace GeometricAlgebraFulcrumLib.Processing.Products.Orthonormal
{
    public static class GaProductOrtCpUtils
    {
        ...
        public static IGasMultivector<double> Cp(this GaSignatureLookup basisSignature, IGasMultivector<double> mv1, IGasMultivector<double> mv2)
        {
            var composer = 
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IGasMultivector<T> Cp<T>(this IGaSignature basisSignature, IGasMultivector<T> mv1, IGasMultivector<T> mv2)
        {
            return basisSignature is GaSignatureEuclidean
                ? mv1.ECp(mv2)
                : mv1.BilinearProduct(mv2, basisSignature.CpSignature);
        }
