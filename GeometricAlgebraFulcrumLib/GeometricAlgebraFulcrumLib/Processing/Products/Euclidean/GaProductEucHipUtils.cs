...
namespace GeometricAlgebraFulcrumLib.Processing.Products.Euclidean
{
    ...
    {
        public static IGasKVector<T> EHip<T>(this IGasKVector<T> mv1, IGasKVector<T> mv2)
        {
            var scalarProcessor = 
                mv1.ScalarProcessor;
        ...
        public static IGasMultivector<T> EHip<T>(this IGasMultivector<T> mv1, IGasMultivector<T> mv2)
        {
            return mv1.BilinearProduct(mv2, GaBasisUtils.EHipSignature);
        }
    }
}