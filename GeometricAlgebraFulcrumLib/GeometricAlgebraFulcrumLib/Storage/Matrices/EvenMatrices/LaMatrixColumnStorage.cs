...
{
    public sealed class LaMatrixColumnStorage<T> :
        ILaMatrixSparseEvenStorage<T>
    {
        ...
        internal LaMatrixColumnStorage(ulong index2, [NotNull] ILaVectorEvenStorage<T> sourceList)
        {
            Key2 = index2;
        ...
        {
            return SourceList.GetIndices().Select(index => new IndexPairRecord(index, Key2));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IEnumerable<IndexPairScalarRecord<T>> GetIndexScalarRecords()
        {
            return SourceList.GetIndexScalarRecords().Select(indexValue => new IndexPairScalarRecord<T>(indexValue.Index, Key2, indexValue.Scalar));
        }
        ...
        public IEnumerable<IndexPairRecord> GetEmptyIndices(ulong maxKey1, ulong maxKey2)
        {
            ...
            return new HashSet<IndexPairRecord>(maxKey.GetKeyPairsInRange())
                .Except(SourceList
                    .GetIndices()
                    .Select(index => new IndexPairRecord(index, Key2))
                );
        }
        ...
        {
            return index2 == Key2 && SourceList.ContainsIndex(index1);
        }
        ...
        public IndexPairRecord GetMinIndex()
        {
            ...
            return new IndexPairRecord(index, Key2);
        }
        ...
        {
            if (index2 == Key2 && SourceList.TryGetScalar(index1, out value))
                return true;
        ...
        public ILaMatrixEvenStorage<T> MapIndices(Func<ulong, ulong, IndexPairRecord> indexMapping)
        {
            ...
            {
                var (index1, index2) = indexMapping(k, Key2);
        ...
        {
            return new LaMatrixDiagonalStorage<T2>(SourceList.MapScalars((index, value) => indexValueMapping(index, Key2, value)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ILaMatrixEvenStorage<T> FilterByIndex(Func<ulong, ulong, bool> indexFilter)
        {
            return new LaMatrixDiagonalStorage<T>(SourceList.FilterByIndex(index => indexFilter(index, Key2)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ILaMatrixEvenStorage<T> FilterByIndexScalar(Func<ulong, ulong, T, bool> indexValueFilter)
        {
            return new LaMatrixDiagonalStorage<T>(SourceList.FilterByIndexScalar((index, value) => indexValueFilter(index, Key2, value)));
        }
        ...
        public bool TryGetCompactStorage(out ILaMatrixEvenStorage<T> evenGrid)
        {
            if (SourceList.TryGetCompactStorage(out var sourceList))
            {
                ...
                evenGrid = count switch
                {
                    ...
                    _ => new LaMatrixColumnStorage<T>(Key2, sourceList)
                };
        ...
        public ILaMatrixGradedStorage<T> ToGradedStorage(Func<ulong, ulong, GradeIndexPairRecord> evenKeyToGradeKeyMapping)
        {
            ...
            foreach (var (k, value) in SourceList.GetIndexScalarRecords())
            {
                var (grade, index1, index2) = evenKeyToGradeKeyMapping(k, Key2);
        ...
        public ILaMatrixGradedStorage<T> ToGradedStorage(Func<ulong, ulong, T, GradeIndexPairScalarRecord<T>> evenIndexScalarToGradeIndexScalarMapping)
        {
            ...
            foreach (var (k, value) in SourceList.GetIndexScalarRecords())
            {
                var (grade, index1, index2, scalar) = evenIndexScalarToGradeIndexScalarMapping(k, Key2, value);
