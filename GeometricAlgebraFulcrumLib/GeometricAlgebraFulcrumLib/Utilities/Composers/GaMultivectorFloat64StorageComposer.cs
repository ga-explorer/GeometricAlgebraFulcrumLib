...
namespace GeometricAlgebraFulcrumLib.Utilities.Composers
{
    ...
    /// </summary>
    public sealed class GaMultivectorFloat64StorageComposer :
        IReadOnlyDictionary<ulong, double>
    {
        ...
        public GaMultivectorFloat64StorageComposer(uint vSpaceDimension)
        {
            Signature = (IGaSignatureLookup)GaSignatureFactory.CreateEuclidean(vSpaceDimension, true);
        }

        public GaMultivectorFloat64StorageComposer(uint positiveCount, uint negativeCount)
        {
            Signature = (IGaSignatureLookup)GaSignatureFactory.Create(positiveCount, negativeCount, true);
        }

        public GaMultivectorFloat64StorageComposer(uint positiveCount, uint negativeCount, uint zeroCount)
        {
            Signature = (IGaSignatureLookup)GaSignatureFactory.Create(positiveCount, negativeCount, zeroCount, true);
        }

        public GaMultivectorFloat64StorageComposer([NotNull] IGaSignatureLookup signature)
        {
            Signature = signature;
        }
        ...
        public IGaMultivectorStorage<double> GetCompactStorage()
        {
            ...
            if (termsCount == 0)
                return GaScalarStorage<double>.ZeroScalar;

            if (termsCount > 1)
                return GaMultivectorSparseStorage<double>.Create(_idScalarDictionary);

            var (id, scalar) =
                _idScalarDictionary.First();

            return id == 0UL
                ? GaScalarStorage<double>.Create(scalar)
                : GaKVectorStorage<double>.Create(id, scalar);
        }
