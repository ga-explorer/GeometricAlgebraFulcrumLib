...
namespace GeometricAlgebraFulcrumLib.Utilities.Extensions
{
    public static class GaVersorsUtils
    {
        public static Tuple<GaFactoredVersor<double>, IGaOutermorphism<double>> GetHouseholderQRDecomposition(this IGaProcessor<double> processor, IGaOutermorphism<double> linearMap, int count)
        {
            ...
            var vectorComposersArray = new LaVectorSparseEvenStorageComposer<double>[count];
            for (var i = 0; i < count; i++)
                vectorComposersArray[i] = 
                    processor.CreateKVectorStorageComposer();
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                foreach (var basisVectorIndex in basisVectorIndicesList)
                {
                    ...
                    var differenceLengthSquared = 
                        processor.ESp(
                            processor.Subtract(
                                basisVectorImage,
                                processor.CreateGaVectorStorage(basisVectorIndex, basisVectorImageNorm)
                            )
                        );
                ...
                var unitVector = processor.Subtract(
                    ...
                    processor.CreateGaVectorStorage(bestBasisVectorIndex, bestBasisVectorImageNorm)
                ).GetVectorPart();
                ...
                foreach (var basisVectorIndex in basisVectorIndicesList)
                {
                    ...
                    mappedBasisVectors[basisVectorIndex] = processor.CreateGaVectorStorage(reflectedVector
                            .IndexScalarList
            ...
            for (var i = 0; i < vectorComposersArray.Length; i++)
            {
                ...
                if (!composer.IsEmpty())
                    linearMapRVectorsDictionary.Add((ulong) i, composer.CreateGaVectorStorage());
            }
        ...
        public static Tuple<GaFactoredVersor<T>, IGaOutermorphism<T>> GetHouseholderQRDecomposition<T>(this IGaProcessor<T> processor, IGaOutermorphism<T> linearMap, int count)
        {
            ...
            //Vector composers to construct r vectors
            var vectorComposersArray = new LaVectorSparseEvenStorageComposer<T>[count];
            for (var i = 0; i < count; i++)
                vectorComposersArray[i] = 
                    processor.CreateKVectorStorageComposer();
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                var unitVector = processor.Subtract(
                    x,
                    processor.CreateGaVectorStorage(i, xNorm)
                ).GetVectorPart();
