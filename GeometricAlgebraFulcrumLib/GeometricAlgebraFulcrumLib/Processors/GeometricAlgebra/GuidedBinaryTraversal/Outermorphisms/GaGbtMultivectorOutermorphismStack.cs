...
namespace GeometricAlgebraFulcrumLib.Processors.GeometricAlgebra.GuidedBinaryTraversal.Outermorphisms
{
        ...
        : GeoGbtStack1
    {
        public static GeoGbtMultivectorOutermorphismStack<T> Create(IReadOnlyList<VectorStorage<T>> basisVectorsMappingsList, GaMultivector<T> mv)
        {
            ...
            return new GeoGbtMultivectorOutermorphismStack<T>(
                basisVectorsMappingsList,
                mv.MultivectorStorage.CreateGbtStack(treeDepth, capacity, mv.GeometricProcessor)
            );
        }

        public static GeoGbtMultivectorOutermorphismStack<T> Create(IReadOnlyList<VectorStorage<T>> basisVectorsMappingsList, IScalarAlgebraProcessor<T> scalarProcessor, IMultivectorStorage<T> mv)
        {
        ...
        private GeoGbtMultivectorOutermorphismStack(IReadOnlyList<VectorStorage<T>> basisVectorsMappingsList, IGeoGbtMultivectorStorageStack1<T> multivectorStack)
        ...
        public IEnumerable<GradeLinVectorStorageScalarRecord<T>> TraverseForScaledKVectors()
        {
            ...
            while (!IsEmpty)
            {
                ...
                if (TosIsLeaf)
                {
                    if (!ScalarProcessor.IsZero(TosValue))
                        yield return new GradeLinVectorStorageScalarRecord<T>(
                            TosKVector.Grade, 
                            TosKVector.GetLinVectorIndexScalarStorage(), 
                            TosValue
                        );
