...
{
    public sealed class GaGridEvenDiagonal<T> :
        IGaGridEvenSparse<T>
    {
        public IGaListEven<T> SourceList { get; }


        internal GaGridEvenDiagonal([NotNull] IGaListEven<T> sourceList)
        {
        ...
        {
            return SourceList.GetKeys().Select(key => new GaRecordKeyPair(key, key));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IEnumerable<GaRecordKeyPairValue<T>> GetKeyValueRecords()
        {
            return SourceList.GetKeyValueRecords().Select(keyValue => new GaRecordKeyPairValue<T>(keyValue.Key, keyValue.Key, keyValue.Value));
        }
        ...
        public IEnumerable<GaRecordKeyPair> GetEmptyKeys(ulong maxKey1, ulong maxKey2)
        {
            ...
            return new HashSet<GaRecordKeyPair>(maxKey.GetKeyPairsInRange())
                .Except(SourceList
                    .GetKeys()
                    .Select(key => new GaRecordKeyPair(key, key))
                );
        }
        ...
        {
            return key1 == key2 && SourceList.ContainsKey(key1);
        }
        ...
        public GaRecordKeyPair GetMinKey()
        {
            ...
            return new GaRecordKeyPair(key, key);
        }
        ...
        {
            if (key1 == key2 && SourceList.TryGetValue(key1, out value))
                return true;
        ...
        {
            return new GaGridEvenDiagonal<T2>(SourceList.MapValues((key, value) => keyValueMapping(key, key, value)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IGaGridEven<T> FilterByKey(Func<ulong, ulong, bool> keyFilter)
        {
            return new GaGridEvenDiagonal<T>(SourceList.FilterByKey(key => keyFilter(key, key)));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IGaGridEven<T> FilterByKeyValue(Func<ulong, ulong, T, bool> keyValueFilter)
        {
            return new GaGridEvenDiagonal<T>(SourceList.FilterByKeyValue((key, value) => keyValueFilter(key, key, value)));
        }
        ...
        public bool TryGetCompactGrid(out IGaGridEven<T> evenGrid)
        {
            if (SourceList.TryGetCompactList(out var sourceList))
            {
                ...
                evenGrid = count switch
                {
                    ...
                    _ => new GaGridEvenDiagonal<T>(sourceList)
                };
        ...
        public IGaGridGraded<T> ToGradedGrid(Func<ulong, ulong, GaRecordGradeKeyPair> evenKeyToGradeKeyMapping)
        {
            ...
            foreach (var (k, value) in SourceList.GetKeyValueRecords())
            {
                var (grade, key1, key2) = evenKeyToGradeKeyMapping(k, k);
