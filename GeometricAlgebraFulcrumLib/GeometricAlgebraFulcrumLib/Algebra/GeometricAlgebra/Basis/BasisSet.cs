...
{
    public sealed record BasisSet : 
        IGeometricAlgebraSpace
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static BasisSet Create(uint vSpaceDimension)
        {
            if (vSpaceDimension < 1)
                throw new ArgumentOutOfRangeException(nameof(vSpaceDimension));

            return new BasisSet(((ulong) vSpaceDimension).GetRange().ToArray());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static BasisSet Create(IReadOnlyList<ulong> basisVectorIndices)
        {
            if (basisVectorIndices.Count < 1)
                throw new ArgumentException(nameof(basisVectorIndices));

            return new BasisSet(basisVectorIndices);
        }
        ...
        private BasisSet([NotNull] IReadOnlyList<ulong> basisVectorIndices)
        {
