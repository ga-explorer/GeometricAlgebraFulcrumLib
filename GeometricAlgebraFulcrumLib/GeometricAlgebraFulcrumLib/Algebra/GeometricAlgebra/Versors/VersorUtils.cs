...
namespace GeometricAlgebraFulcrumLib.Algebra.GeometricAlgebra.Versors
{
    ...
    {
        public static Tuple<PureVersorsSequence<double>, IGaOutermorphism<double>> GetHouseholderQRDecomposition(this IGeometricAlgebraProcessor<double> processor, IGaOutermorphism<double> linearMap, int count)
        {
            ...
            var vectorComposersArray = new LinVectorSparseStorageComposer<double>[count];
            ...
            return new Tuple<PureVersorsSequence<double>, IGaOutermorphism<double>>(
                linearMapQ,
                linearMapR
            );
        }

        public static Tuple<PureVersorsSequence<T>, IGaOutermorphism<T>> GetHouseholderQRDecomposition<T>(this IGeometricAlgebraProcessor<T> processor, IGaOutermorphism<T> linearMap, int count)
        {
            ...
            //Vector composers to construct r vectors
            var vectorComposersArray = new LinVectorSparseStorageComposer<T>[count];
            for (var i = 0; i < count; i++)
                vectorComposersArray[i] =
                    processor.CreateVectorStorageComposer();
            ...
            return new Tuple<PureVersorsSequence<T>, IGaOutermorphism<T>>(
                linearMapQ,
                linearMapR
            );
        }
