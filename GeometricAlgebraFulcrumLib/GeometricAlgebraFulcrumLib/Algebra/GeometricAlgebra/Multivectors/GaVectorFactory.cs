...
namespace GeometricAlgebraFulcrumLib.Algebra.GeometricAlgebra.Multivectors
{
    public static class GaVectorFactory
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static GaVector<T> CreateUnitVector<T>(this IGeometricAlgebraEuclideanProcessor<T> geometricProcessor, int basisIndex1, int basisIndex2, Scalar<T> angle)
        {
            var e1 = geometricProcessor.CreateVectorBasis(basisIndex1);
            var e2 = geometricProcessor.CreateVectorBasis(basisIndex2);

            return angle.Cos() * e1 + angle.Sin() * e2;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static GaVector<T> CreatePhasor<T>(this IGeometricAlgebraEuclideanProcessor<T> processor, int basisIndex, Scalar<T> magnitude, Scalar<T> angle)
        {
            Debug.Assert(
            ...
            return magnitude * processor.CreateUnitVector(
                basisIndex, 
                basisIndex + n, 
                angle
            );
