...
namespace GeometricAlgebraFulcrumLib.Algebra.SignalProcessing
{
    public class ScalarFourierSeries
    {
        ...
        private static ScalarFourierSeries Create(IReadOnlyList<Complex> complexSamples, ScalarSignalFloat64 scalarSignal, double snrThreshold, IEnumerable<int> frequencyIndexList)
        {
            var sampleCount = complexSamples.Count;
            var scalingFactor = Math.Sqrt(sampleCount);
        ...
        public static IReadOnlyList<double> GetPaddedSignal(IReadOnlyList<double> signalSamples, double samplingRate, int trendSampleCount)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ScalarFourierSeries Create(ScalarSignalFloat64 signalSamples, double snrThreshold, IEnumerable<int> frequencyIndexList)
        {
            var complexSamples = 
                GetFourierArray(signalSamples);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private ScalarFourierSeries([NotNull] Dictionary<double, ScalarFourierSeriesTerm> termsDictionary)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ScalarFourierSeries GetInterpolatorDerivative(int degree = 1)
        {
            var termsDictionary = 
                _termsDictionary
                    .Values
                    .ToDictionary(
                        term => term.Frequency, 
                        term => term.GetTermDerivative(degree)
                    );
        ...
        public double GetScalarDt(double parameterValue, int degree = 1)
        {
            return _termsDictionary.Values.Aggregate(
                0d, 
                (current, term) => current + term.GetScalarDt(parameterValue, degree)
            );
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public IEnumerable<double> GetScalars(IEnumerable<double> parameterValueList)
        {
            return parameterValueList.Select(GetScalar);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ScalarSignalFloat64 GetSignal(ScalarSignalFloat64 tValuesSignal)
        {
            return tValuesSignal.Select(GetScalar).CreateSignal(tValuesSignal.SamplingRate);
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ScalarSignalFloat64 GetSignalDt(ScalarSignalFloat64 tValuesSignal)
        {
