...
using GeometricAlgebraFulcrumLib.Core.GeometricAlgebra.Extended.Generic.LinearMaps.Rotors;
...
using GeometricAlgebraFulcrumLib.Core.GeometricAlgebra.Extended.Generic.Subspaces;
...
public static class XGaFrameComposerUtils
{
    ...
    public static XGaBasisVectorFrame<T> CreateBasisVectorFrame<T>(this XGaProcessor<T> processor, int vSpaceDimensions)
    {
        return XGaBasisVectorFrame<T>.Create(
            vSpaceDimensions.GetRange<XGaVector<T>>(processor.VectorTerm)
        );
    }
