...
namespace GeometricAlgebraFulcrumLib.Core.Modeling.Graphics.Computers.Reflections;
...
public sealed class GcTriangleReflector3D
{
    ...
    public LinFloat64Vector3D ReflectPointVa(ILinFloat64Vector3D point)
    {
        ...
        var n2 = n.VectorENormSquared();
        var d = -n.VectorESp(Triangle.GetPoint1());
        var p = point.ToLinVector3D();

        var result = p - 2 * (n.VectorESp(p) + d) / n2 * n;
    ...
    public LinFloat64Vector3D ReflectPoint(ILinFloat64Vector3D point)
    {
        ComputePointReflectionCounter.Begin();
        ...
        return LinFloat64Vector3D.Create(pointImageX, pointImageY, pointImageZ);
    }
}