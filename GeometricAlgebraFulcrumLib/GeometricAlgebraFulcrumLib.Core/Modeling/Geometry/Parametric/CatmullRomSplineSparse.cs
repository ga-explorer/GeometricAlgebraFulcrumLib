...
using GeometricAlgebraFulcrumLib.Core.Modeling.Geometry.Parametric.SpaceND.Curves.CatmullRom;
...
public sealed class CatmullRomSplineSparse
{
    ...
    private readonly List<LinFloat64Vector> _pointList;

    public CatmullRomSplineType CurveType { get; }

    public bool IsClosed { get; }

    public IEnumerable<LinFloat64Vector> ControlPoints
        => _pointList;
    ...
    internal CatmullRomSplineSparse(IEnumerable<LinFloat64Vector> inputPointList, CatmullRomSplineType curveType, bool isClosed)
    {
        CurveType = curveType;
        IsClosed = isClosed;
        _pointList = new List<LinFloat64Vector>(inputPointList);

        LinFloat64Vector endPoint1, endPoint2;
    ...
    public LinFloat64Vector GetPoint(double parameterValue)
    {
        // Handle edge cases
        if (parameterValue <= _knotList[0])
            return _pointList[0];
    ...
    public LinFloat64Vector GetTangent(double parameterValue)
    {
    ...
    public LinFloat64Vector GetSecondDerivative(double parameterValue)
    {
        if (parameterValue is <= 0d or >= 1d)
            return LinFloat64Vector.Create(
                GetPointItemIndexList()
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public LinFloat64Vector GetUnitTangent(double parameterValue)
    {
        return GetTangent(parameterValue).ToUnitLinVector();
    }
}