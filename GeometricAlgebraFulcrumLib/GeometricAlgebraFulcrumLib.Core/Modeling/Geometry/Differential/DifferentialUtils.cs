...
namespace GeometricAlgebraFulcrumLib.Core.Modeling.Geometry.Differential;
...
public static class DifferentialUtils
{
    ...
    public static IEnumerable<LinFloat64Vector2D> GetBezierSmoothingPoints(this IEnumerable<double> yInputList, IEnumerable<double> xInputList, int bezierDegree, bool assumeUniformX)
    {
        ...
        {
            var p0 = LinFloat64Vector2D.Create((Float64Scalar)xArray[j - 1], (Float64Scalar)yArray[j - 1]);
            var p1 = LinFloat64Vector2D.Create((Float64Scalar)xArray[j], (Float64Scalar)yArray[j]);
            var p2 = LinFloat64Vector2D.Create((Float64Scalar)xArray[j + 1], (Float64Scalar)yArray[j + 1]);
        ...
        if (assumeUniformX)
            return xOutputList.Count.GetRange().Select(idx =>
                LinFloat64Vector2D.Create((Float64Scalar)xOutputList[idx], (Float64Scalar)yOutputList[idx])
            );
        ...
        return xOutputList.Count.GetRange().Select(idx =>
            LinFloat64Vector2D.Create((Float64Scalar)xUniformArray[idx], (Float64Scalar)yUniformArray[idx])
        );
    }
