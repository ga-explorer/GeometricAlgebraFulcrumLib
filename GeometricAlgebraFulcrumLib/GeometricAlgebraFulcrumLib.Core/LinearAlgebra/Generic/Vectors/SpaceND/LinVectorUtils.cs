...
namespace GeometricAlgebraFulcrumLib.Core.LinearAlgebra.Generic.Vectors.SpaceND;
...
public static class LinVectorUtils
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static LinAngle<T> GetEuclideanAngle<T>(this LinVector<T> v1, LinVector<T> v2, bool assumeUnitVectors = false)
    {
        var v12Sp = v1.ESp(v2);
    ...
    public static LinVector<T> GetBisector<T>(this LinVector<T> v1, LinVector<T> v2, bool assumeEqualNormVectors = false)
    {
        ...
        return (v1 + v2).Divide(scalarProcessor.TwoValue);
    }
