...
public class CclMatlabCodeGenerator :
    CclLanguageCodeGeneratorBase,
    ICclImperativeCodeGenerator,
    ICclObjectOrientedCodeGenerator
{
    internal CclMatlabCodeGenerator()
    {
        LanguageInfo = CclMatlabUtils.Matlab4Info;
        ...
        if (code.IsOperator)
        {
            ...
            if (opHeaderSpecs.Position == SteOperatorPosition.Infix)
            {
                ...
                foreach (var argCode in code.Arguments)
                {
                    ...
                    if (argCode.Precedence < code.Precedence && !argCode.IsAtomic)
                    {
                        TextComposer.Append("(");
                        argCode.AcceptVisitor(this);
                        TextComposer.Append(")");
            ...
            if (ReferenceEquals(argCode1, null) == false)
            {
                ...
                if (argCode1.Precedence < code.Precedence && !argCode1.IsAtomic)
                {
                    TextComposer.Append("(");
