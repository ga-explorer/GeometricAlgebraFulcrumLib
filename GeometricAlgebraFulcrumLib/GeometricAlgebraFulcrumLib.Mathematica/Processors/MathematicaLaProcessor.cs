...
{
    public sealed class LaMathematicaProcessor
        : ILaSymbolicProcessor<Expr>
    {
        public static LaMathematicaProcessor DefaultProcessor { get; }
            = new LaMathematicaProcessor();
        ...
        private LaMathematicaProcessor()
        {
        }
        ...
        public ILaVectorEvenStorage<Expr> MatrixRowToVector(Expr matrix, int rowIndex)
        {
        ...
        public ILaVectorEvenStorage<Expr> MatrixColumnToVector(Expr matrix, int colIndex)
        {
        ...
        public ILaMatrixEvenStorage<Expr> MatrixToArray(Expr matrix)
        {
        ...
        public Expr CreateMatrix(ILaMatrixEvenStorage<Expr> array)
        {
            return array.ToArray().ArrayToMatrixExpr().Simplify();
        }

        public Expr CreateRowVectorMatrix(ILaVectorEvenStorage<Expr> array)
        {
        ...
        public Expr CreateRowVectorMatrix(ILaMatrixEvenStorage<Expr> array, int rowIndex)
        {
        ...
        public Expr CreateColumnVectorMatrix(ILaVectorEvenStorage<Expr> array)
        {
        ...
        public Expr CreateColumnVectorMatrix(ILaMatrixEvenStorage<Expr> array, int columnIndex)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Expr GetScalarFromInteger(int value)
        {
            return ScalarProcessor.GetScalarFromInteger(value);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Expr GetScalarFromFloat64(double value)
        {
            return ScalarProcessor.GetScalarFromFloat64(value);
        }
        ...
        public int MatrixEigenDecomposition(Expr matrix, out Tuple<Expr, ILaVectorEvenStorage<Expr>>[] realPairs, out Tuple<Expr, ILaVectorEvenStorage<Expr>>[] imagPairs)
        {
            ...
            realPairs = new Tuple<Expr, ILaVectorEvenStorage<Expr>>[count];
            imagPairs = new Tuple<Expr, ILaVectorEvenStorage<Expr>>[count];

            for (var i = 0; i < count; i++)
            {
                realPairs[i] = new Tuple<Expr, ILaVectorEvenStorage<Expr>>(
                    realEigenValues[i],
                    realEigenVectors[i].Args.LaVectorDenseStorage()
                );

                imagPairs[i] = new Tuple<Expr, ILaVectorEvenStorage<Expr>>(
                    imagEigenValues[i],
                    imagEigenVectors[i].Args.LaVectorDenseStorage()
                );
            }
        ...
        public Pair<ILaVectorEvenStorage<Expr>>[] MatrixEigenVectors(Expr matrix)
        {
            ...
            var eigenVectorTuples = new Pair<ILaVectorEvenStorage<Expr>>[count];

            for (var i = 0; i < count; i++)
            {
                eigenVectorTuples[i] = new Pair<ILaVectorEvenStorage<Expr>>(
                    realEigenVectors[i].Args.LaVectorDenseStorage(),
                    imagEigenVectors[i].Args.LaVectorDenseStorage()
                );
            }
