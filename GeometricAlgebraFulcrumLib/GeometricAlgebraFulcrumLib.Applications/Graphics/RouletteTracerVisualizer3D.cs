    ...
    GrBabylonJsSnapshotComposer3D
{
    ...
    private readonly List<GrRouletteCurve3D> _rouletteCurveList = new List<GrRouletteCurve3D>();
    public IReadOnlyList<GrRouletteCurve3D> RouletteCurveList 
        => _rouletteCurveList;

    private readonly List<GrParametricCurveTree3D> _sampledCurveList = new List<GrParametricCurveTree3D>();
    public IReadOnlyList<GrParametricCurveTree3D> SampledCurveList 
        => _sampledCurveList;



    public RouletteTracerVisualizer3D(IReadOnlyList<double> cameraAlphaValues, IReadOnlyList<double> cameraBetaValues, IGraphicsC1ArcLengthCurve3D fixedCurve, IGraphicsC1ArcLengthCurve3D movingCurve, IReadOnlyList<GeneratorPoint> generatorPointList, double rouletteDistance, int fixedCurveFrameCount, int movingCurveFrameCount)
        : base(cameraAlphaValues, cameraBetaValues)
    {
        ...
        _rouletteCurveList.AddRange(
            GeneratorPointList.Select(p => 
                new GrRouletteCurve3D(FixedCurve, MovingCurve, p.Point, RouletteDistance)
            )
        );
        ...
        _sampledCurveList.AddRange(
            _rouletteCurveList.Select(curve =>
                curve.CreateSampledCurve3D(
                    ...
                    new GrParametricCurveTreeOptions3D(3.DegreesToAngle(), 3, 16)
                )
            )
        );
