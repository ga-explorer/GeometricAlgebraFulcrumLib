...
public static class LinUnilinearMapComposerUtils
{
    ...
    {
        return ToLinUnilinearMap(vectorList, scalarProcessor);
    }
    
    public static LinUnilinearMap<T> ToLinUnilinearMap<T>(this IEnumerable<LinVector<T>> vectorList, IScalarProcessor<T> scalarProcessor)
    {
        var indexVectorDictionary = new Dictionary<int, LinVector<T>>();
