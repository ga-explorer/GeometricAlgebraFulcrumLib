...
namespace GeometricAlgebraFulcrumLib.Algebra.Polynomials;
    ...
    IReadOnlyList<Scalar<T>>
{
    ...
    public static PolynomialFunction<T> Create(IScalarProcessor<T> scalarProcessor, params T[] monomialCoefficients)
    {
        return new PolynomialFunction<T>(
            scalarProcessor, 
            monomialCoefficients.Select(t => t ?? scalarProcessor.ZeroValue).ToArray()
        );
    }
    ...
    public Scalar<T> this[int index] 
        => _monomialCoefficientArray[index].ScalarFromValue(ScalarProcessor);

    public IEnumerable<Scalar<T>> MonomialCoefficients
        => _monomialCoefficientArray.Select(c => c.ScalarFromValue(ScalarProcessor));
    ...
    {
        var value = ScalarProcessor.ZeroValue;

        for (var i = 0; i < _monomialCoefficientArray.Length; i++)
        {
            var ci = _monomialCoefficientArray[i];
            var ti = ScalarProcessor.Power(t, i).ScalarValue;

            value = ScalarProcessor.Add(value, ScalarProcessor.Times(ci, ti).ScalarValue).ScalarValue;
        }
    ...
    {
        return GetValue(t.ScalarValue).ScalarFromValue(t.ScalarProcessor);
    }
    ...
    public T GetValueDt1(T t)
    {
        ...
        for (var i = 1; i < _monomialCoefficientArray.Length; i++)
        {
            ...
            value = ScalarProcessor.Add(
                value, 
                ScalarProcessor.Times(ci, ti).ScalarValue
            ).ScalarValue;
        }
    ...
    {
        return GetValueDt1(t.ScalarValue).ScalarFromValue(t.ScalarProcessor);
    }
    ...
    {
        return GetValueDt2(t.ScalarValue).ScalarFromValue(t.ScalarProcessor);
    }
    ...
    private PolynomialFunction<T> GetFirstDerivative()
    {
        ...
        for (var i = 2; i < _monomialCoefficientArray.Length; i++)
        {
            monomialCoefficientArray[i - 1] =
                ScalarProcessor.Times(i, _monomialCoefficientArray[i]).ScalarValue;
        }
    ...
    public IEnumerator<Scalar<T>> GetEnumerator()
    {
        return _monomialCoefficientArray
            .Select(c => c.ScalarFromValue(ScalarProcessor))
            .GetEnumerator();
    }
