...
public sealed class XGaGbtKVectorStorageStack1<T>
    : XGaGbtStack1, IXGaGbtMultivectorStorageStack1<T>
{
    public static XGaGbtKVectorStorageStack1<T> Create(int capacity, int treeDepth, IScalarProcessor<T> scalarProcessor, XGaKVector<T> multivectorStorage)
    {
        return new XGaGbtKVectorStorageStack1<T>(capacity, treeDepth, scalarProcessor, multivectorStorage);
    }
    ...
    public XGaKVector<T> XGaKVector { get; }

    public IScalarProcessor<T> ScalarProcessor { get; }

    public XGaMultivector<T> Multivector 
        => XGaKVector;
    ...
    private XGaGbtKVectorStorageStack1(int capacity, int treeDepth, IScalarProcessor<T> scalarProcessor, XGaKVector<T> multivectorStorage)
        : base(capacity, treeDepth, IndexSet.EmptySet)
    {
        ScalarProcessor = scalarProcessor;
        XGaKVector = multivectorStorage;
        ActiveGradesBitPattern = IndexSet.CreateUnit(multivectorStorage.Grade);
