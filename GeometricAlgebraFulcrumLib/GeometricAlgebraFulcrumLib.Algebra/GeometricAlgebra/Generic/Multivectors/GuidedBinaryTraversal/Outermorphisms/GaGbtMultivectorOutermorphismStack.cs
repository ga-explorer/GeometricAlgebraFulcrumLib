...
public record XGaScalarKVectorRecord<T>(T Scalar, XGaKVector<T> KVector);

public sealed class XGaGbtMultivectorOutermorphismStack<T>
    : XGaGbtStack1
{
    public static XGaGbtMultivectorOutermorphismStack<T> Create(IReadOnlyList<XGaVector<T>> basisVectorsMappingsList, XGaMultivector<T> mv)
    {
        var treeDepth = Math.Max(1, mv.VSpaceDimensions);
        var capacity = treeDepth + 1;

        return new XGaGbtMultivectorOutermorphismStack<T>(
            basisVectorsMappingsList,
            mv.CreateGbtStack(treeDepth, capacity)
        );
    }

    public static XGaGbtMultivectorOutermorphismStack<T> Create(IReadOnlyList<XGaVector<T>> basisVectorsMappingsList, IScalarProcessor<T> scalarProcessor, XGaMultivector<T> mv)
    {
        var treeDepth = Math.Max(1, mv.VSpaceDimensions);
        var capacity = treeDepth + 1;
    ...
    private XGaGbtMultivectorOutermorphismStack(IReadOnlyList<XGaVector<T>> basisVectorsMappingsList, IXGaGbtMultivectorStorageStack1<T> multivectorStack)
    ...
    public IEnumerable<XGaScalarKVectorRecord<T>> TraverseForScaledKVectors()
    {
        ...
        while (!IsEmpty)
        {
            ...
            if (TosIsLeaf)
            {
                if (!ScalarProcessor.IsZero(TosValue))
                    yield return new XGaScalarKVectorRecord<T>(
                        TosValue,
                        TosKVector
                    );
