...
public static class XGaVectorComposerUtils
{
    ...
    public static XGaVector<T> Vector<T>(this XGaProcessor<T> processor, int termsCount, Func<int, double> indexToScalarFunc)
    {
    ...
    public static XGaVector<T> Vector<T>(this XGaProcessor<T> processor, int termsCount, Func<int, string> indexToScalarFunc)
    {
        var scalarProcessor = processor.ScalarProcessor;
        var composer = processor.CreateComposer();
    ...
    {
        var basisScalarDictionary =
            new SingleItemDictionary<IndexSet, T>(basisVectorId.ToUInt64IndexSet(), processor.ScalarProcessor.OneValue);
    ...
    {
        return processor.VectorTerm(idScalarPair.Key.ToUInt64IndexSet(), idScalarPair.Value);
    }
