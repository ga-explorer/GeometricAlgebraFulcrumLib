...
public static class XGaRotorComposerUtils
{
    ...
    /// <returns></returns>
    public static XGaPureRotor<T> CreateParametricPureRotor3D<T>(this XGaVector<T> sourceVector, XGaVector<T> targetVector, LinPolarAngle<T> angleTheta)
    {
        var processor = sourceVector.Processor;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaScaledPureRotor<T> CreateScaledParametricPureRotor3D<T>(this XGaVector<T> sourceVector, XGaVector<T> targetVector, LinPolarAngle<T> angleTheta, T scalingFactor)
    {
        return sourceVector
    ...
    /// <returns></returns>
    public static XGaPureRotor<T> CreatePureRotor<T>(this XGaBivector<T> rotationBlade, LinPolarAngle<T> rotationAngle)
    {
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaPureRotor<T> CreateGivensRotor<T>(this XGaProcessor<T> processor, int i, int j, LinPolarAngle<T> rotationAngle)
    {
        ...
        return XGaPureRotor<T>.Create(
            cosHalfAngle.ScalarValue,
            processor.BivectorTerm(i, j, sinHalfAngle.ScalarValue)
        );
    }
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaScaledPureRotor<T> CreateScaledGivensRotor<T>(this XGaProcessor<T> processor, int i, int j, LinPolarAngle<T> rotationAngle, T scalingFactor)
    {
        Debug.Assert(i >= 0 && j != i);
        ...
        var bivectorPart = s * processor.BivectorTerm(i, j, sinHalfAngle.ScalarValue);

        return XGaScaledPureRotor<T>.Create(scalarPart.ScalarValue, bivectorPart);
    }
    ...
    /// <returns></returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaScaledPureRotor<T> CreateScaledPureRotor3D<T>(this XGaProcessor<T> processor, IScalar<T> r0, IScalar<T> r12, IScalar<T> r13, IScalar<T> r23)
    {
        return XGaScaledPureRotor<T>.Create(
            processor
                .CreateComposer()
    ...
    public static XGaRotor<T> CreateRotorToFrame<T>(this XGaBasisVectorFrame<T> sourceFrame, XGaBasisVectorFrame<T> targetFrame)
    {
        ...
        var rotorMultivector =
            mvFrame2
                ...
                .Aggregate(
                    (XGaMultivector<T>) processor.MultivectorZero,
                    (mv1, mv2) => mv1.Add(mv2)
                );
    ...
    {
        var v1 = 
            processor.VectorSymmetricUnit(vSpaceDimensions);

        var v2 = processor.VectorTerm(
            vSpaceDimensions - 1
        );
