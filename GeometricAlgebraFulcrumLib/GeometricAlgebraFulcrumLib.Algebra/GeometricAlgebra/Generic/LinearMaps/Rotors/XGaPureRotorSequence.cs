...
public sealed class XGaPureRotorSequence<T> : 
    XGaRotorBase<T>, 
    IXGaOutermorphismSequence<T>,
    IReadOnlyList<XGaPureRotor<T>>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaPureRotorSequence<T> CreateIdentity(XGaProcessor<T> metric)
    {
        return new XGaPureRotorSequence<T>(
            [metric.CreateIdentityRotor()]
        );
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaPureRotorSequence<T> Create(params XGaPureRotor<T>[] rotorsList)
    {
        return new XGaPureRotorSequence<T>(rotorsList);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static XGaPureRotorSequence<T> Create(IEnumerable<XGaPureRotor<T>> rotorsList)
    {
        return new XGaPureRotorSequence<T>(rotorsList.ToImmutableArray());
    }

    public static XGaPureRotorSequence<T> CreateFromOrthonormalEuclideanFrames(XGaVectorFrame<T> sourceFrame, XGaVectorFrame<T> targetFrame, bool fullRotorsFlag = false)
    {
        ...
        return new XGaPureRotorSequence<T>(rotorsSequence);
    }

    public static XGaPureRotorSequence<T> CreateFromOrthonormalEuclideanFrames(XGaVectorFrame<T> sourceFrame, XGaVectorFrame<T> targetFrame, int[] sequenceArray)
    {
        Debug.Assert(targetFrame.Count == sourceFrame.Count);
        Debug.Assert(sourceFrame.IsOrthonormal() && targetFrame.IsOrthonormal());
        Debug.Assert(sourceFrame.HasSameHandedness(targetFrame));
    ...
    public static XGaPureRotorSequence<T> CreateFromEuclideanFrames(int baseSpaceDimensions, XGaVectorFrame<T> sourceFrame, XGaVectorFrame<T> targetFrame)
    {
        Debug.Assert(targetFrame.Count == sourceFrame.Count);
    ...
    private XGaPureRotorSequence(IReadOnlyList<XGaPureRotor<T>> rotorsList)
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public XGaPureRotorSequence<T> GetSubSequence(int startIndex, int count)
    {
        return new XGaPureRotorSequence<T>(
