        ...
        IGaSignature
    {
        ...
        private GaSignature(int positiveCount, int negativeCount, int zeroCount)
        {
            ...
            _positiveMask = PositiveCount > 0 
                ? UnsignedLongBitUtils.CreateFullMask(PositiveCount)
                : 0UL;

            _negativeMask = NegativeCount > 0
                ? UnsignedLongBitUtils.CreateFullMask(NegativeCount) << PositiveCount
                : 0UL ;

            _zeroMask = ZeroCount > 0
                ? UnsignedLongBitUtils.CreateFullMask(ZeroCount) << (PositiveCount + NegativeCount)
                : 0UL;
