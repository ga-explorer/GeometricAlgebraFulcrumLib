...
namespace GeometricAlgebraLib.Processing.Products
{
    public sealed class GaBilinearProductsKVectorsIterator<T>
    {
        public static GaBilinearProductsKVectorsIterator<T> Create(IGaMultivector<T> mv1, IGaMultivector<T> mv2)
        {
            return new(
                mv1.ScalarProcessor,
        ...
        public static GaBilinearProductsKVectorsIterator<T> Create(IGaScalarProcessor<T> scalarProcessor, IReadOnlyDictionary<int, IGaKVectorStorage<T>> storageList1, IReadOnlyDictionary<int, IGaKVectorStorage<T>> storageList2)
        {
            return new(
                scalarProcessor,
                storageList1,
        ...
        private readonly GaBilinearProductsTermsIterator<T> _termsIterator 
            = GaBilinearProductsTermsIterator<T>.Create();


        public IGaScalarProcessor<T> ScalarProcessor { get; }

        public IReadOnlyDictionary<int, IGaKVectorStorage<T>> StoragesDictionary1 { get; }

        public IReadOnlyDictionary<int, IGaKVectorStorage<T>> StoragesDictionary2 { get; }


        private GaBilinearProductsKVectorsIterator([NotNull] IGaScalarProcessor<T> scalarProcessor, [NotNull] IReadOnlyDictionary<int, IGaKVectorStorage<T>> storageList1, [NotNull] IReadOnlyDictionary<int, IGaKVectorStorage<T>> storageList2)
        {
        ...
        private IEnumerable<IGaKVectorStorage<T>> GetESpKVector2(int grade1)
        {
            if (StoragesDictionary2.TryGetValue(grade1, out var storage2))
                yield return storage2;
        }

        private IEnumerable<IGaKVectorStorage<T>> GetELcpKVectors2(int grade1)
        {
            return StoragesDictionary2.Values.Where(storage => 
                storage.Grade - grade1 >= 0
            );
        }

        private IEnumerable<IGaKVectorStorage<T>> GetERcpKVectors2(int grade1)
        {
            return StoragesDictionary2.Values.Where(storage => 
                grade1 - storage.Grade >= 0
            );
        }
